...
public:
    ...
    template<const DENDRITE_UINT order, const TALYFEMLIB::ElemType elemType>
    void syncCoords(const DENDRITE_REAL *coords, TALYFEMLIB::GRID *grid, const int * ids);
...
template<const DENDRITE_UINT order, const TALYFEMLIB::ElemType elemType>
void TalyDendroSync::syncCoords(const DENDRITE_REAL *coords, TALYFEMLIB::GRID *grid, const int * ids){
#if(DIM == 2)

    if(order == 1) {
        static constexpr int NUM_NODES = (elemType == TALYFEMLIB::ElemType::kElem2dTriangle) ? 3 : 4;
        for (unsigned int i = 0; i < NUM_NODES; i++) {
            grid->node_array_[i]->setCoor(coords[ids[i] * DIM], coords[ids[i] * DIM + 1]);
        }
    ...
    if(order == 1) {
        int NUM_NODES = 0;
        switch (elemType) {
            case TALYFEMLIB::ElemType::kElemPyramid:
                NUM_NODES  = 5;
                break;
            case TALYFEMLIB::ElemType::kElem3dHexahedral:
                NUM_NODES  = 8;
                break;
            case TALYFEMLIB::ElemType::kElem3dTetrahedral:
                NUM_NODES  = 4;
                break;
            default:
                throw std::runtime_error("Unknown type of element.Cannot be reached");

        }
        for (unsigned int i = 0; i < NUM_NODES; i++) {
            grid->node_array_[i]->setCoor(coords[ids[i] * DIM], coords[ids[i] * DIM + 1],coords[ids[i] * DIM + 2]);
        }
