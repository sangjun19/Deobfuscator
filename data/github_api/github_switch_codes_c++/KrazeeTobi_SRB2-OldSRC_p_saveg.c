...
// Logan: the answer: SH4 (Dreamcast) and ARM (WINCE/PPC)
...
static void P_ArchivePlayers(void)
{
	...
	for (i = 0; i < MAXPLAYERS; i++)
	{
		...
		WRITEBYTE(save_p, players[i].charflags);
		WRITEBYTE(save_p, players[i].supercolor);
...
static void P_UnArchivePlayers(void)
{
	...
	for (i = 0; i < MAXPLAYERS; i++)
	{
		...
		players[i].charflags = READBYTE(save_p);
		players[i].supercolor = READBYTE(save_p);
...
static void P_NetArchivePlayers(void)
{
	...
	for (i = 0; i < MAXPLAYERS; i++)
	{
		...
		WRITELONG(save_p, players[i].laps);
...
static void P_NetUnArchivePlayers(void)
{
	...
	for (i = 0; i < MAXPLAYERS; i++)
	{
		...
		players[i].laps = READLONG(save_p); // Number of laps (optional)
		...
		players[i].snowbuster = READBYTE(save_p); // Snow Buster upgrade!
...
static void P_NetArchiveThinkers(void)
{
	...
	for (th = thinkercap.next; th != &thinkercap; th = th->next)
	{
		...
		else if (th->function.acp1 == (actionf_p1)T_MoveFloor)
		{
			...
			PADSAVEP();
			savefloor = (floormove_t *)save_p;
			memcpy(savefloor, th, sizeof (*savefloor));
	...
	for (;;)
	{
		...
		switch (tclass)
		{
			...
			case tc_floor:
				PADSAVEP();
				{
					floormove_t *loadfloor;

					loadfloor = Z_Malloc(sizeof (*loadfloor), PU_LEVEL, NULL);
					memcpy(loadfloor, save_p, sizeof (*loadfloor));
			...
			case tc_noenemies:
				PADSAVEP();
				{
					...
					elevator->sector = &sectors[(size_t)elevator->sector];
					elevator->sourceline = &lines[(size_t)elevator->sourceline];
					elevator->thinker.function.acp1 = (actionf_p1)T_NoEnemiesSector;
					P_AddThinker(&elevator->thinker);
				}
				break;
...
static void P_RelinkPointers(void)
{
		...
		currentthinker = currentthinker->next)
	{
		if (currentthinker->function.acp1 == (actionf_p1)P_MobjThinker)
		{
			...
			{
				mobj->player->axis1 = FindNewPosition((size_t)mobj->player->axis1);
				if (!mobj->player->axis1)
					CONS_Printf("axis1 not found on %d\n", mobj->type);
			}
			if (mobj->player && mobj->player->axis2)
			{
				mobj->player->axis2 = FindNewPosition((size_t)mobj->player->axis2);
...
static void P_NetUnArchiveSpecials(void)
{
	...
	switch (globalweather)
	{
		...
		case 2:
			...
			break;
		case 3:
			if (curWeather == PRECIP_RAIN)
				curWeather = PRECIP_NONE;
			P_SwitchWeather(globalweather);
			break;
		default: // 0
			if ((curWeather == PRECIP_SNOW || curWeather == PRECIP_RAIN || curWeather == PRECIP_STORM))
				P_SwitchWeather(globalweather);
			break;
	}
}
