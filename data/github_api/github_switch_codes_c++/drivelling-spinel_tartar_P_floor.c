...
void T_MoveFloor(floormove_t* floor)
{
  ...
  if (!(leveltime&7) && !silentmove(floor->sector))     // make the floormove sound
    S_StartSoundName((mobj_t *)&floor->sector->soundorg, info_sound_stnmov);
  ...
  if (res == pastdest)    // if destination height is reached
  {
    ...
    if(!silentmove(floor->sector)) //sf: silentmove
        S_StartSoundName((mobj_t *)&floor->sector->soundorg, info_sound_pstop);
  }
}
...
void T_MoveElevator(elevator_t* elevator)
{
  ...
  if (!(leveltime&7) && !silentmove(elevator->sector))
    S_StartSoundName((mobj_t *)&elevator->sector->soundorg, info_sound_stnmov);
  ...
  if (res == pastdest)            // if destination height acheived
  {
    ...
    if(!silentmove(elevator->sector))   //sf: silentmove
        S_StartSoundName((mobj_t *)&elevator->sector->soundorg, info_sound_pstop);
  }
}
  ...
  while ((secnum = P_FindSectorFromLineTag(line,secnum)) >= 0)
  {
    ...
    do
    {
      ...
      for (i = 0;i < sec->linecount;i++)
      {
        ...
        if(demo_version < 200 || demo_version >= 300 ||
           (demo_version >= 203 && demo_version <= 205))
          height += stairsize; // jff 6/19/98 prevent double stepsize

        // if sector's floor already moving, look for another
        if (P_SectorActive(floor_special,tsec)) //jff 2/22/98
	  continue;

        if(demo_version == 202 || (demo_version > 205 && demo_version < 300))
          height += stairsize; // jff 6/19/98 increase height AFTER continue
