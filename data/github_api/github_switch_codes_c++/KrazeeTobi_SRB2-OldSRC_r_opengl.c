...
//-----------------------------------------------------------------------------
//
// $Id: r_opengl.c,v 1.57 2001/12/31 13:47:46 hurdler Exp $
//
...
//-----------------------------------------------------------------------------
/// \file
/// \brief OpenGL v1.1 API for Sonic Robo Blast 2
...
EXPORT void HWRAPI( SetBlend ) ( FBITFIELD PolyFlags )
{
	...
	if ( Xor & ( PF_Blending|PF_Occlude|PF_NoTexture|PF_Modulated|PF_NoDepthTest|PF_Decal|PF_Invisible|PF_NoAlphaTest ) )
	{
		...
		else if ( Xor&PF_ReadOnlyDepth )
		{
			if ( PolyFlags & PF_ReadOnlyDepth )
                                    ...
                                    FBITFIELD     PolyFlags )
{
	...
	if ( (PolyFlags & PF_NoDepthTest) || (PolyFlags & PF_ReadOnlyDepth) )
		glDepthMask(GL_TRUE);
}
...
EXPORT void HWRAPI( DrawMD2 ) (int *gl_cmd_buffer, md2_frame_t *frame, FTransform *pos, float scale)
{
	...
	while (val != 0)
	{
		...
		while (count--)
		{
			...
			glTexCoord2f (s, t);
			glVertex3f (frame->vertices[pindex].vertex[0]/2.0f,
...
EXPORT void HWRAPI( SetTransform ) (FTransform *stransform)
{
	...
	if (stransform)
	{
		...
		glRotatef(stransform->anglex       , 1.0f, 0.0f, 0.0f);
		glRotatef(stransform->angley+270.0f, 0.0f, 1.0f, 0.0f);
