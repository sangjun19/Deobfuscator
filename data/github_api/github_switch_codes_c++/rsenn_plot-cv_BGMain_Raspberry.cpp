...
//全局变量
cv::Mat g_srcImage, g_srcImage_fliped;
cv::Mat g_filterImage;
cv::Mat g_grayImage;
...
cv::Mat hsvImage_base_template_pre, hsvImage_base_template;
cv::Mat hsvImage_base; //直方图对比变量
...
void
on_ContoursChangeHalf(int, void*) {
...
saveConfig() {
  ...
  cvWriteString(fs,
                ...
                "定义格子的横向精度，是一个百分比值1-"
                "100，同时需要将modeScan设置为1，建议取可以被100整除的值");
  ...
  cvWriteString(fs,
                ...
                "定义原始图像翻转的形式，0: 沿横向x轴翻转, 1: 沿竖向y轴翻转, -1: x、y轴同时翻转, "
                "-2不进行翻转");
...
main(int argc, char** argv) {
  ...
  if(modeGrid2) { //进入格子检测的采用动点检测的方法
    ...
    while(1) {
      ...
      bgs->process(g_srcImage, img_mask,
...
*/
void
setGridStatus(int x, int y, int grid_array[][100]) {
...
on_MouseHandle(int event, int x, int y, int flags, void* param) {
  ...
  cv::Mat& image = *(cv::Mat*)param;     //参数为原始一帧图像
