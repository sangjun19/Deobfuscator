...
void initialize(int argc, char **argv) {
  ...
  usage_error_msg += " -o <output_file.csv> -n <neuron_file.isf> "
      ...
      "\t-t   Simulation time (optional)\n"
      "\t-e   Time step (optional)\n"
  ...
  insilico::engine::time_specified = false;
  insilico::engine::timestep_specified = false;
  insilico::engine::frequency_specified = false;
  insilico::engine::probability_specified = false;
  ...
  for(unsigned iter = 0; iter < cmds.size(); iter+=2) {
    ...
    switch(cmd.at(1)) {
      ...
      case 'd':
        ...
        break;
      case 't':
      std::cerr << "Simulation time specified: " << cmds[iter+1] << '\n';
      insilico::engine::simulation_time = insilico::string_to_double(cmds[iter+1]);
      insilico::engine::time_specified = true;
      break;
      case 'f':
      std::cerr << "Frequency: " << cmds[iter+1] << '\n';
      insilico::engine::frequency_poisson = insilico::string_to_double(cmds[iter+1]);
      insilico::engine::frequency_specified = true;
      break;
      case 'p':
      std::cerr << "Release Probability: " << cmds[iter+1] << '\n';
      insilico::engine::release_probability = insilico::string_to_double(cmds[iter+1]);
      insilico::engine::probability_specified = true;
      break;
      case 'h':
      std::cerr << "Time step: " << cmds[iter+1] << '\n';
      insilico::engine::time_step = insilico::string_to_double(cmds[iter+1]);
      insilico::engine::timestep_specified = true;
      break;
      default:
