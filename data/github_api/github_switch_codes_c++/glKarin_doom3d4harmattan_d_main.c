...
void D_PostEvent(event_t *ev)
{
  ...
  if (gametic < 3) return;
	...
	/*
		 M_Responder -> for menu
		 HU_Responder -> for input text
		 ST_Responder -> for state or cheat
		 AM_Responder -> for auto map mode
		 G_Responder -> for game
		 F_Responder -> end
	*/
#endif
  M_Responder(ev) ||
