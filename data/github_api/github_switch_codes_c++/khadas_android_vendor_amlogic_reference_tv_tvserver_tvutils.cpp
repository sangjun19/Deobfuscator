...
int getPropertyInt(const char *key, int def)
{
    ...
    const sp<SystemControlClient> &sws = getSystemControlService();
    if (sws != nullptr) {
        ret = sws->getPropertyInt(key, def);
    }
    return ret;
}
...
int tvSetDisplaymode(int source_input, int mode, int isSave)
{
    ...
    const sp<SystemControlClient> &sws = getSystemControlService();
    if (sws != nullptr) {
        s32Ret = sws->setDisplayMode(source_input, mode, isSave);
        return s32Ret;
    }
    return -1;
}
...
int tvGetDisplaymode(int source_input)
{
    ...
    const sp<SystemControlClient> &sws = getSystemControlService();
    if (sws != nullptr) {
        s32Ret = sws->getDisplayMode(source_input);
        return s32Ret;
    }
    return -1;
}
