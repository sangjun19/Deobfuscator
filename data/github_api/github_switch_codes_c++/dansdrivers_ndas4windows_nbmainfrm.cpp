...
//	pMainFrame->m_listDevices.push_back(pNBNdasDevice);
	...
	if (pNBNdasDevice->UnitDevicesCount() == 0) {
		...
		BaseInfo.UnitNo = 0;
		pNBNdasDevice->UnitDeviceAdd(&BaseInfo, 0);
	}
...
BOOL CMainFrame::CheckCommandAvailable(CNBDevice *pDevice, int wID)
{
	if(!pDevice->GetCommandAbility(wID)) {
		...
		// device Status was not up-to-date. Refresh whole devices.
		RefreshStatus();
		return FALSE;	
	}
	return TRUE;
...
BOOL CMainFrame::RefreshStatus(BOOL DelayUpdate)
{
	BOOL bResult;
...
//	UIEnableForDevice(NULL, IDM_TOOL_SPAREREMOVE);
	...
	m_wndRefreshProgress.SetRange32(0, (DelayUpdate?3:0) + m_listDevices.size() * 3 );
			...
			if ((*ld)[i] == NULL) { // Found missing member.
					...
					itDevice != m_listDevices.end(); itDevice++)
				{
					...
					if (pNdasDevice->IsThisDevice(ld->RAID_INFO()->Members[i].DeviceId.Node, ld->RAID_INFO()->Members[i].DeviceId.VID)) 
					{
						...
						pUnitDevice = pMissingNdasDevice->UnitDeviceAdd(&BaseInfo, ld->RAID_INFO()->Members[i].Flags);
						if (pUnitDevice)
				...
				(*ld)[i]->SetRaidStatus(ld->RAID_INFO()->Members[i].Flags);
			}
	...
	for(NBUnitDevicePtrList::iterator itUnusedUnit = UnusedUnits.begin();
		itUnusedUnit != UnusedUnits.end(); itUnusedUnit++) {
		pUnitDevice = *itUnusedUnit;
		pLogicalDevice = new CNBLogicalDevice();
		m_listLogicalDevices.push_back(pLogicalDevice);
		ATLTRACE(_T("new CNBLogicalDevice(%p) : %s\n"), pLogicalDevice, pUnitDevice->GetName());
...
//	UIEnableForDevice(pDevice, IDM_TOOL_SPAREREMOVE);
	UIEnableForDevice(pDevice, IDM_TOOL_RESET_BIND_INFO);
...
void CMainFrame::OnMigrate(UINT wNotifyCode, int wID, HWND hwndCtl)
{
	...
	pLogicalDevice->UpdateStatus();

	if (!CheckCommandAvailable(pDevice, wID)) {
		return;
	}
	...
	((*pLogicalDevice)[0])->InitConnectionInfo(&ci, FALSE);

	AutoCursor l_auto_cursor(IDC_WAIT);
	BOOL bResult = NdasOpMigrate(&ci);
...
void CMainFrame::OnSpareAdd(UINT wNotifyCode, int wID, HWND hwndCtl)
{
	...
	(*pLogicalDevice)[0]->InitConnectionInfo(&ci, FALSE);
	pUnitDevice->InitConnectionInfo(&ci_spare, FALSE);
...
void CMainFrame::OnReplaceDevice(UINT wNotifyCode, int wID, HWND hwndCtl)
{
	...
	ReplaceIndex = pOldUnitDevice->GetSequence();
	...
	pLogicalDevice->InitConnectionInfo(&ci, FALSE);
	pNewUnitDevice->InitConnectionInfo(&ci_new, FALSE);

	AutoCursor l_auto_cursor(IDC_WAIT);

	BOOL bResults = NdasOpReplaceDevice(
		&ci, &ci_new, ReplaceIndex);
...
out:
	...
	// pLogicalDevice is not updated yet.
	pNewUnitDevice->HixChangeNotify(pGetNdasHostGuid());
...
void CMainFrame::OnRemoveFromRaid(UINT wNotifyCode, int wID, HWND hwndCtl)
{
	...
	if (pLogicalDevice->GetType() == NMT_RAID1R3) {
		...
		for(i=0;i<pLogicalDevice->DevicesTotal();i++) {
			...
			if ((UnitRaidStatus & NDAS_RAID_MEMBER_FLAG_ONLINE) &&
				(UnitRaidStatus & NDAS_RAID_MEMBER_FLAG_ACTIVE)) {
				if (!(UnitRaidStatus & (
					NDAS_RAID_MEMBER_FLAG_OUT_OF_SYNC |
					NDAS_RAID_MEMBER_FLAG_INVALID_MEMBER))) {
					ValidMemberExist = TRUE;
...
{
	// Just convert to basic disk. Same as OnResetBindInfo except warning message
...
void CMainFrame::OnResetBindInfo(UINT wNotifyCode, int wID, HWND hwndCtl)
{
	...
	bResults = NdasOpBind(1, &ci, NMT_SINGLE,0);
	l_auto_cursor.Release();
out:
	pLogicalDevice->HixChangeNotify(pGetNdasHostGuid());
...
void CMainFrame::OnUseAsMember(UINT wNotifyCode, int wID, HWND hwndCtl)
{
	...
	//
	// Check disk is replaceable. At least one member should be valid after replacing.
	//
	if (pLogicalDevice->GetType() == NMT_RAID1R3) {
		BOOLEAN ValidMemberExist = FALSE;
		UINT i;
		// Check at least one active member is not out-of-sync.
		for(i=0;i<pLogicalDevice->DevicesTotal();i++) {
			DWORD UnitRaidStatus;
		...
		if (!ValidMemberExist) {
			CString strMsg;
			strMsg.LoadString(IDS_ERROR_RAID_BROKEN);
			CString strTitle;
			strTitle.LoadString(IDS_APPLICATION);
			MessageBox(
				strMsg,
