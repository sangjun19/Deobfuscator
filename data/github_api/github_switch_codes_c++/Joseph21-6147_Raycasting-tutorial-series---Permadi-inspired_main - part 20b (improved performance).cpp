...
//
// Implementation of part 20 b - fractional wall heights with improved performance
//
...
/* Short description
   -----------------
   This implementation is a follow up of implementation part 20 a. See the description of that implementation as well (and check on
   the differences between that cpp file and this one).
   
   I noticed the performance had gradually degraded with each feature that was added to the raycaster. So I decided to put som time in
   performance improvement:
     * created look up versions of sine and cosine functions and applied them everywhere possible
     * experimented with faster versions of atan2(), but improvement was hardly measurable, so did not proceed with it
     * built in a check on the wall rendering (where the atan2() is and where some calculation is needed to decide 
       which face the wall was hiet), reducing the calculations in rendering a wall
     * did some test with profiling - after all this, FPS more or less has doubled, which suffices for now.
