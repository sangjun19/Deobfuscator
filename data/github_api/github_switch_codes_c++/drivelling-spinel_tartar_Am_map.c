  ...
  for (i=0;i<numlines;i++)
  {
    ...
    if (ddt_cheating || (lines[i].flags & ML_MAPPED))
    {
      ...
      if (!lines[i].backsector)
      {
        ...
        else if (mapcolor_tele && lines[i].special && !comp[comp_livemap])
      ...
      else
      {
        ...
        {          
          AM_drawMline(&l, (mapcolor_tele && lines[i].special && !comp[comp_livemap]) ?
                           mapcolor_tele : mapcolor_fchg); // floor level change
        }
        else if (lines[i].backsector->ceilingheight !=
                  lines[i].frontsector->ceilingheight)
        {
          AM_drawMline(&l, (mapcolor_tele && lines[i].special && !comp[comp_livemap]) ?
                           mapcolor_tele : mapcolor_cchg); // ceiling level change
        }
  ...
  int  colorrange)
{
  ...
  for (i=0;i<numsectors;i++)
  {
    ...
    {
      if(t->player ||
         (ddt_cheating != 2 && (comp[comp_livemap] || !t->args[1]))
        )
      {
        t = t->snext;
        continue;
      ...
      if(ddt_cheating != 2 && map_secret_after && P_IsSecret(&sectors[i]))
      {
        t = t->snext;
        continue;
      }
      ...
      if(!comp[comp_livemap] && ddt_cheating != 2 && (t->flags & MF_SPECIAL))
      {
        AM_drawLineCharacter
        (
          cross_mark,
          ...
          16<<FRACBITS,
          t->angle,
          mapcolor_frnd != -1 ? mapcolor_frnd : mapcolor_sprt,
          t->x,
          t->y
        );
        t = t->snext;
      ...
      if(t->flags & MF_FRIEND)
         c = mapcolor_frnd;
      else if(ddt_cheating != 2 && !comp[comp_livemap])
         c = sentient(t) ? RED - (t->args[1] >> 4) : mapcolor_sngl;
...
void AM_Drawer (void)
{
  ...
  AM_drawWalls();
  AM_drawPlayers();
  if (ddt_cheating==2 || !comp[comp_livemap])
