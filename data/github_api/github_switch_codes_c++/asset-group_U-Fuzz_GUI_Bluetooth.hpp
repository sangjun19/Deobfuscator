// Repository: asset-group/U-Fuzz
// File: src/gui/GUI_Bluetooth.hpp
...
#define GUI_LOG4_NAME ICON_FA_BLUETOOTH_B " BTStack"
#define GUI_LOG5_NAME ICON_FA_HEARTBEAT " Monitor"
#define GUI_CONFIG_PATH "/tmp/bt_fuzz_gui.bin"
#define GUI_LOG_LIMIT 8000   // Max lines
#define GUI_SUMMARY_LIMIT 17 // Max lines for summary
...
inline void MainView()
{
    ...
    if (ImGui::BeginMenuBar()) {
        if (ImGui::BeginMenu("File")) {
            if (ImGui::MenuItem(ICON_FA_WINDOW_CLOSE " Close " GUI_APP_NAME, NULL, false, true)) {
                glfwSetWindowShouldClose(window, 1);
            }
            ImGui::EndMenu();
        }
        ImGui::Dummy(ImVec2(ImGui::GetContentRegionAvailWidth() - 180, 0));
...
inline void gui_disable_items(bool val)
{
...
int GUI_Init(int argc, char **argv, bool enable = true)
{
    ...
    // Check if gui is disabled via command line
    for (size_t i = 0; i < argc; i++) {
        if (string(argv[i]) == "--no-gui")
            return 0;
        if (string(argv[i]) == "--scan")
            return 0;
        else if (string(argv[i]) == "--list-exploits")
            return 0;

        if (string(argv[i]) == "--test-webview")
    ...
    pthread_setname_np(GUI_Thread_Handler->native_handle(), "gui_bluetooth");

    sem_wait(&sem_webview_done);
    sem_destroy(&sem_webview_done);

    uint8_t wait_gui_count = 0;
    while (!gui_enabled) {
        usleep(10000UL); // Sleep 10ms
        if (wait_gui_count++ > 100) {
            LOG1("GUI Delayed");
            return -1;
        }
    }
...
static MemoryEditor GuiMemEditor;
#endif