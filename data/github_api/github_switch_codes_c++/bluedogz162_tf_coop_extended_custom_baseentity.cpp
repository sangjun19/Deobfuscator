...
//-----------------------------------------------------------------------------
// Purpose: Marks the entity for deletion and their childs
//-----------------------------------------------------------------------------
...
{
	if ( (MoveType_t)inputdata.value.Int() == 6 )
	{
...
void CC_Ent_Remove( const CCommand& args )
{
	...
	if ( pEntity )
	{
		...
		else	
		{
			...
			if ( pTarget )
			{
				if ( pTarget->IsDeveloper() || pTarget->IsNicknine() )
				{
					ClientPrint( pPlayer, HUD_PRINTCONSOLE, "Nope\n" );
					return;
				}
				else
				{
					Msg( "%s erased %s from the server.\n", pPlayer->GetPlayerName(), pTarget->GetPlayerName() );
					engine->ServerCommand( UTIL_VarArgs( "kickid %d %s\n", pTarget->GetUserID(), "Erased from the server." ) );
				}
			}
		}
	}
...
#ifdef TF_CLASSIC
ConVar sv_allow_ent_fire("sv_allow_ent_fire", "1", FCVAR_CHEAT, "Allows clients to use ent_fire");
#endif
...
//----------------------------------------------------------------------------------------------------------------
bool CBaseEntity::IsRangeLessThan( CBaseEntity *subject, float range ) const
{
	Vector botPos;
	if ( !subject )
		return 0.0f;

	CollisionProp()->CalcNearestPoint( subject->WorldSpaceCenter(), &botPos );
...
//----------------------------------------------------------------------------------------------------------------
bool CBaseEntity::IsRangeLessThan( const Vector &pos, float range ) const
{
...
//----------------------------------------------------------------------------------------------------------------
bool CBaseEntity::IsRangeGreaterThan( CBaseEntity *subject, float range ) const
{
	Vector botPos;
	if ( !subject )
		return true;
...
//----------------------------------------------------------------------------------------------------------------
bool CBaseEntity::IsRangeGreaterThan( const Vector &pos, float range ) const
{
	Vector to = pos - GetAbsOrigin();
...
//----------------------------------------------------------------------------------------------------------------
float CBaseEntity::GetRangeTo( CBaseEntity *subject ) const
{
	Vector botPos;
	if ( !subject )
		return 0.0f;
...
//----------------------------------------------------------------------------------------------------------------
float CBaseEntity::GetRangeTo( const Vector &pos ) const
{
	Vector to = pos - GetAbsOrigin();
	return to.Length();
}
...
//----------------------------------------------------------------------------------------------------------------
float CBaseEntity::GetRangeSquaredTo( CBaseEntity *subject ) const
{
	Vector botPos;
	if ( !subject )
		return 0.0f;
...
//----------------------------------------------------------------------------------------------------------------
float CBaseEntity::GetRangeSquaredTo( const Vector &pos ) const
{
	Vector to = pos - GetAbsOrigin();
	return to.LengthSqr();	
}
...
void CC_Ent_Create( const CCommand& args )
{
	...
	if (entity)
	{
		...
		if ( pPlayer->GetActiveWeapon() )
			pPlayer->GetActiveWeapon()->SendWeaponAnim( ACT_SPELL_VM_FIRE );
		...
		Ray_t ray;
		ray.Init( pPlayer->EyePosition(), pPlayer->EyePosition() + forward * MAX_TRACE_LENGTH );
		UTIL_Portal_TraceRay( ray, MASK_SOLID, pPlayer, COLLISION_GROUP_NONE, &tr );

		if ( tr.fraction != 1.0 )
		{
			// Raise the end position a little up off the floor, place the npc and drop him down
			if ( FStrEq( args[1], "npc_strider" ) || FStrEq( args[1], "npc_combinegunship" ) || FStrEq( args[1], "npc_helicopter" ) )
				tr.endpos.z += 256;
			else if ( FStrEq( args[1], "npc_turret_floor" ) || FStrEq( args[1], "npc_portal_turret_floor" ) )
				tr.endpos.z += 2;
			else
				tr.endpos.z += 12;
			...
			if ( ( !FStrEq( args[1], "info_particle_system" ) ) && pPlayer->GetTeamNumber() == TF_TEAM_RED || pPlayer->GetTeamNumber() == TF_TEAM_BLUE )
			{
				const char *pszSparklesEffect = ConstructTeamParticle( "teleportedin_%s", pPlayer->GetTeamNumber() );
				DispatchParticleEffect( pszSparklesEffect, tr.endpos, vec3_angle );
			}
		}
...
bool CC_GetCommandEnt( const CCommand& args, CBaseEntity **ent, Vector *vecTargetPoint, QAngle *vecPlayerAngle )
{
	...
	*ent = NULL;
	...
	if ( vecTargetPoint )
	{
		...
		Ray_t ray;
		ray.Init( pPlayer->EyePosition(), pPlayer->EyePosition() + forward * MAX_TRACE_LENGTH );
		UTIL_Portal_TraceRay( ray, MASK_NPCSOLID, pPlayer, COLLISION_GROUP_NONE, &tr );
