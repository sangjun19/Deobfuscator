// Repository: mrwonko/Jedi-Academy-Renaissance
// File: codemp/game/bg_panimate.cpp
...
animation_t *BG_AnimsetAlloc(void)
{
	...
	bgAllAnims[bgNumAllAnims].anims = (animation_t *) BG_Alloc< mod >(sizeof(animation_t)*MAX_TOTALANIMATIONS);
...
void BG_AnimsetFree(animation_t *animset)
{
 ...
 //none of this is actually needed serverside. Could just be moved to cgame code but it's here since it used to tie in a lot with the anim loading stuff.
		...
		case AEV_SOUND:			//# animID AEV_SOUND framenum soundpath randomlow randomhi chancetoplay
			...
			if(lowestVal && highestVal)
			{
				...
				for ( n = lowestVal, num = AED_SOUNDINDEX_START; n <= highestVal && num <= AED_SOUNDINDEX_END; n++, num++ )
				{
					...
					{
						animEvents[curAnimEvent].eventData[num] = S_RegisterSound( va( stringData, n ) );
					}
				}
			...
			else
			{
				...
				{
					animEvents[curAnimEvent].eventData[AED_SOUNDINDEX_START] = S_RegisterSound( stringData );
				}
#ifndef FINAL_BUILD
		...
		case AEV_EFFECT:		//# animID AEV_EFFECT framenum effectpath boltName
			...
			{//actually are specifying a bolt to use
				...
				{ //eh, whatever. no dynamic stuff, so this will do.
					animEvents[curAnimEvent].stringData = (char *) BG_Alloc< mod >(2048);
				}
			...
			//animEvent->eventData[AED_BOLTINDEX] = g_trap->G2API_AddBolt( &cent->gent->ghoul2[cent->gent->playerModel], animEvent->stringData );
			//get probability
			token = COM_Parse( text_p );
			if ( !token )
			{//WARNING!  BAD TABLE!
	...
	while ( 1 )
	{
		...
		{//NOTE: you REALLY should NOT do this after the main block of UPPERSOUNDS and LOWERSOUNDS
			...
			if ( include_filename != NULL )
			{
				...
				BG_ParseAnimationEvtFile< mod >( fullIPath, animFileIndex, forcedIndex );
				bg_animParseIncluding--;
			}
		}
		...
		{
			ParseAnimationEvtBlock< mod >( as_filename, torsoAnimEvents, animations, &upper_i, &text_p );
		}

		else if ( !Q_stricmp(token,"LOWEREVENTS") )	// A batch of lower sounds
		{
			ParseAnimationEvtBlock< mod >( as_filename, legsAnimEvents, animations, &lower_i, &text_p );
		}
	}
...
template int BG_ParseAnimationEvtFile< moduleType::cgame >( const char *as_filename, int animFileIndex, int eventFileIndex );
...
template int BG_ParseAnimationFile< moduleType::game >( const char *filename, animation_t *animset, qboolean isHumanoid );
template int BG_ParseAnimationFile< moduleType::cgame >( const char *filename, animation_t *animset, qboolean isHumanoid );
...
static void BG_StartLegsAnim( playerState_t *ps, int anim )
{
	...
	else if( mod == moduleType::game  && g_entities[ ps->clientNum ].s.legsAnim == anim )
...
void PM_ContinueLegsAnim( int anim ) {
	...
	BG_StartLegsAnim< mod >( pm->ps, anim );
}
template void PM_ContinueLegsAnim< moduleType::game >( int anim );
template void PM_ContinueLegsAnim< moduleType::cgame >( int anim );
...
template void PM_ForceLegsAnim< moduleType::game >( int anim );
template void PM_ForceLegsAnim< moduleType::cgame >( int anim );
...
void BG_StartTorsoAnim( playerState_t *ps, int anim )
{
	...
	else if ( mod == moduleType::game && g_entities[ps->clientNum].s.torsoAnim == anim)
	{ //toggled anim to one anim then back to the one we were at previously in
		//one frame, indicating that anim should be restarted.
		BG_FlipPart(ps, SETANIM_TORSO);
	}
...
{
    BG_StartTorsoAnim< mod >(pm->ps, anim);
}
template void PM_StartTorsoAnim< moduleType::game >( int anim );
template void PM_StartTorsoAnim< moduleType::cgame >( int anim );
...
void BG_SaberStartTransAnim( int clientNum, int saberAnimLevel, int weapon, int anim, float *animSpeed, int broken )
{
	if ( anim >= BOTH_A1_T__B_ && anim <= BOTH_ROLL_STAB )
	{
		if ( weapon == WP_SABER )
		{
			saberInfo_t *saber = BG_MySaber< mod >( clientNum, 0 );
			if ( saber
				&& saber->animSpeedScale != 1.0f )
			{
				*animSpeed *= saber->animSpeedScale;
			}
			saber = BG_MySaber< mod >( clientNum, 1 );
			if ( saber
				&& saber->animSpeedScale != 1.0f )
			{
				*animSpeed *= saber->animSpeedScale;
...
template void BG_SaberStartTransAnim< moduleType::game >( int clientNum, int saberAnimLevel, int weapon, int anim, float *animSpeed, int broken );
template void BG_SaberStartTransAnim< moduleType::cgame >( int clientNum, int saberAnimLevel, int weapon, int anim, float *animSpeed, int broken );
template void BG_SaberStartTransAnim< moduleType::ui>( int clientNum, int saberAnimLevel, int weapon, int anim, float *animSpeed, int broken );
					 ...
					 int setAnimParts,int anim,int setAnimFlags)
{
	...
	BG_SaberStartTransAnim< mod >(ps->clientNum, ps->fd.saberAnimLevel, ps->weapon, anim, &editAnimSpeed, ps->brokenLimbs);
...
{
	BG_SetAnimFinal< mod >(pm->ps, pm->animations, setAnimParts, anim, setAnimFlags);
}
...
//	}
	...
	BG_SetAnimFinal< mod >(ps, animations, setAnimParts, anim, setAnimFlags);
}
template void BG_SetAnim< moduleType::game >( playerState_t *ps, animation_t *animations, int setAnimParts, int anim, int setAnimFlags );
template void BG_SetAnim< moduleType::cgame >( playerState_t *ps, animation_t *animations, int setAnimParts, int anim, int setAnimFlags );

template< moduleType mod >
void PM_SetAnim(int setAnimParts,int anim,int setAnimFlags)
{
	BG_SetAnim< mod >(pm->ps, pm->animations, setAnimParts, anim, setAnimFlags);
}
template void PM_SetAnim< moduleType::game >( int setAnimParts, int anim, int setAnimFlags );
template void PM_SetAnim< moduleType::cgame >( int setAnimParts, int anim, int setAnimFlags );
