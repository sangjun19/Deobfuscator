// Repository: ruixuejianfei/BitScalableDeepHash
// File: src/caffe/layers/pooling_layer.cu
...
__global__ void MaxPoolForward(const int nthreads, const Dtype* bottom_data,
		...
		const int pooled_height, const int pooled_width, const int ksize,
...
__global__ void StoPoolForwardTrain(const int nthreads,
		...
		const int pooled_width, const int ksize, const int stride,
		float* rand_idx, Dtype* top_data) {
	int index = threadIdx.x + blockIdx.x * blockDim.x;
	if (index < nthreads) {
		int pw = index % pooled_width;
		int ph = (index / pooled_width) % pooled_height;
...
void PoolingLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
	...
	case LayerParameter_PoolMethod_STOCHASTIC:
		if (Caffe::phase() == Caffe::TRAIN) {
			...
			CURAND_CHECK(
					curandGenerateUniform(Caffe::curand_generator(),
...
__global__ void MaxPoolBackward(const int nthreads, const Dtype* bottom_data,
		const Dtype* top_data, const Dtype* top_diff, const int num,
		const int channels, const int height, const int width,
		const int pooled_height, const int pooled_width, const int ksize,
		const int stride, Dtype* bottom_diff) {
	int index = threadIdx.x + blockIdx.x * blockDim.x;
	if (index < nthreads) {
		...
		Dtype gradient = 0;
		Dtype bottom_datum = bottom_data[((n * channels + c) * height + h)
				* width + w];
...
template<typename Dtype>
__global__ void StoPoolBackward(const int nthreads, const float* rand_idx,
