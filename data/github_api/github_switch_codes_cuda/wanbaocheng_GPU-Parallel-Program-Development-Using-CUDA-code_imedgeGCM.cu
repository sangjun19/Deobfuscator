// Repository: wanbaocheng/GPU-Parallel-Program-Development-Using-CUDA-code
// File: CUDA-CODE/imedgeGCM.cu
...
__global__
void
ThresholdKernel(uch *ImgResult, double *ImgGrad, double *ImgTheta, ui Hpixels, ui Vpixels, ui ThreshLo, ui ThreshHi) {
    ui ThrPerBlk = blockDim.x;
    ui MYbid = blockIdx.x;
    ui MYtid = threadIdx.x;
    ui MYgtid = ThrPerBlk * MYbid + MYtid;
    ...
    if ((MYrow < 1) || (MYrow > Vpixels - 2) || (MYcol < 1) || (MYcol > Hpixels - 2)) {
        ...
        if (G <= L) {                        // no edge
            ...
            if ((T < -67.5) || (T > 67.5)) {
                ...
                PIXVAL = ((ImgGrad[MYpixIndex - Hpixels + 1] > H) || (ImgGrad[MYpixIndex + Hpixels - 1] > H)) ? EDGE
                                                                                                              : NOEDGE;
...
int main(int argc, char **argv) {
    ...
    switch (argc) {
        ...
        case 7:
            BWKN = atoi(argv[6]);
        case 6:
            ThreshHi = atoi(argv[5]);
        case 5:
            ThreshLo = atoi(argv[4]);
        case 4:
            ThrPerBlk = atoi(argv[3]);
        case 3:
            strcpy(InputFileName, argv[1]);
            strcpy(OutputFileName, argv[2]);
            break;
        default:
            printf("\n\nUsage:   %s InputFilename OutputFilename [ThrPerBlk] [ThreshLo] [ThreshHi] [BWKernel=1-9] [GaussKernel=1-9] [SobelKernel=1-9] [ThresholdKernel=1-9]",
                   ProgName);
    ...
    if ((BWKN < 1) || (BWKN > 9) || (GaussKN < 1) || (GaussKN > 9) || (SobelKN < 1) || (SobelKN > 9) ||
        (ThresholdKN < 1) || (ThresholdKN > 9)) {
    ...
    cudaEventCreate(&time2BW);
    cudaEventCreate(&time2Gauss);
    cudaEventCreate(&time2Sobel);
    ...
    switch (BWKN) {
        case 1:
            ...
            break;
        case 2:
            BWKernel2 <<< dimGrid2D, ThrPerBlk >>>(GPUBWImg, GPUImg, IPH, RowBytes);
            strcpy(BWKernelName, "BWKernel2:       Pre-computed values and 2D blocks");
            break;
        case 3:
            BWKernel3 <<< dimGrid2D4, ThrPerBlk >>>(GPUBWImg, GPUImg32, IPH, RowInts);
            strcpy(BWKernelName, "BWKernel3:       Calculates 4 pixels (3 int) at a time");
            break;
        default:
            printf("...... BW Kernel Number=%d ... NOT IMPLEMENTED .... \n", BWKN);
    ...
    switch (SobelKN) {
        case 1:
            ...
            break;
        default:
            printf("...... Sobel Kernel Number=%d ... NOT IMPLEMENTED .... \n", SobelKN);
    ...
    switch (ThresholdKN) {
        case 1:
            ThresholdKernel <<< NumBlocks, ThrPerBlk >>>(GPUResultImg, GPUGradient, GPUTheta, IPH, IPV, ThreshLo,
                                                         ThreshHi);
            strcpy(ThresholdKernelName, "ThresholdKernel: Everything is passed into the kernel");
            break;
        default:
            printf("...... Threshold Kernel Number=%d ... NOT IMPLEMENTED .... \n", ThresholdKN);
    ...
    cudaEventSynchronize(time2BW);
    cudaEventSynchronize(time2Gauss);
    ...
    printf("%s %s %s %u %d %d %d %d %d %d   [Launched %u BLOCKS, %u BLOCKS/ROW]\n",
           ProgName, InputFileName, OutputFileName, ThrPerBlk, ThreshLo, ThreshHi, BWKN, GaussKN, SobelKN, ThresholdKN,
           NumBlocks, BlkPerRow);
    if (GaussKN == 7) {
    ...
    printf("--------------------------------------------------------------------------------------------------\n");
    printf("              CPU->GPU Transfer =%7.2f ms  ...  %4d MB  ...  %6.2f GB/s\n", tfrCPUtoGPU, DATAMB(IMAGESIZE),
           DATABW(IMAGESIZE, tfrCPUtoGPU));
    printf("              GPU->CPU Transfer =%7.2f ms  ...  %4d MB  ...  %6.2f GB/s\n", tfrGPUtoCPU, DATAMB(IMAGESIZE),
           DATABW(IMAGESIZE, tfrGPUtoCPU));
    printf("----------------------------------------------------------------------------\n");
    printf("       BW Kernel Execution Time =%7.2f ms  ...  %4d MB  ...  %6.2f GB/s\n", kernelExecTimeBW,
           DATAMB(GPUDataTfrBW), DATABW(GPUDataTfrBW, kernelExecTimeBW));
    printf("    Gauss Kernel Execution Time =%7.2f ms  ...  %4d MB  ...  %6.2f GB/s\n", kernelExecTimeGauss,
           DATAMB(GPUDataTfrGauss), DATABW(GPUDataTfrGauss, kernelExecTimeGauss));
    printf("    Sobel Kernel Execution Time =%7.2f ms  ...  %4d MB  ...  %6.2f GB/s\n", kernelExecTimeSobel,
           DATAMB(GPUDataTfrSobel), DATABW(GPUDataTfrSobel, kernelExecTimeSobel));
    printf("Threshold Kernel Execution Time =%7.2f ms  ...  %4d MB  ...  %6.2f GB/s\n", kernelExecTimeThreshold,
           DATAMB(GPUDataTfrThresh), DATABW(GPUDataTfrThresh, kernelExecTimeThreshold));
    printf("----------------------------------------------------------------------------\n");
    printf("         Total Kernel-only time =%7.2f ms  ...  %4d MB  ...  %6.2f GB/s\n", totalKernelTime,
           DATAMB(GPUDataTfrKernel), DATABW(GPUDataTfrKernel, totalKernelTime));
    printf("   Total time with I/O included =%7.2f ms  ...  %4d MB  ...  %6.2f GB/s\n", totalTime,
           DATAMB(GPUDataTfrTotal), DATABW(GPUDataTfrTotal, totalTime));
    printf("----------------------------------------------------------------------------\n");
    ...
    cudaEventDestroy(time2BW);
    cudaEventDestroy(time2Gauss);
    cudaEventDestroy(time2Sobel);
