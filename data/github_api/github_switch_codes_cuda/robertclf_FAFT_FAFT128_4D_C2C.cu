// Repository: robertclf/FAFT
// File: FAFT_64-points_C2C/FAFT128_4D_C2C.cu
...
__global__ void FAFT128_C2C_ax3_dev( float2 *dataComplex, float dx, float delta, int segment, float normFactor )
{
    int tid = threadIdx.x;
    size_t sector = blockIdx.y*gridDim.x + blockIdx.x;
    ...
    load128_half_C2C_ax3( 8, y, dataComplex, 16 );
			
    GENERAL_FAFT128( y, dx, delta, segment, tid );
    
    if ( normFactor != 0.0f ) normalize( 16, y, normFactor );
    
    store128_half_C2C_ax3<8>( y, dataComplex, 16 );
}

extern "C" int FAFT128_4D_C2C( float2 *dataComplex, float dx, float delta, int segment, int axes, float normFactor )
{
	int success = 1;
	dim3 grid_C2C(64*64, 64);
	...
	switch(axes)
	{
		...
		case 3:
			FAFT128_C2C_ax3_dev<<< grid_C2C, 16 >>>( dataComplex, dx, delta, segment, normFactor );			
			break;
