// Repository: koichishirahata/GraphGPU
// File: src/Mars/sample_apps/RWR/MarsLib.cu
...
void InitMapReduce(Spec_t* spec)
{
  Spec_t* g_spec = spec;
	...
	if (g_spec->numMapReduce <= 0)
	  g_spec->numMapReduce = 1;
...
//--------------------------------------------------
int startMap(Spec_t* spec, int num)
{
  //  DoLog("Map gettid : %d", syscall(SYS_gettid));
  //  DoLog("Map pthread_self : %d", pthread_self());
	...
	//1, get map input data on host
	//-------------------------------------------------------
	int	h_inputRecordCount = 0;
	int	h_inputKeysBufSize = 0;
	int	h_inputValsBufSize = 0;
	char*	h_inputKeys = NULL;
	char*	h_inputVals = NULL;
	int4*	h_inputOffsetSizes = NULL;


	h_inputRecordCount = g_spec->inputRecordCount;
	if (num == 1) {
	  h_inputKeysBufSize = g_spec->inputOffsetSizes[h_inputRecordCount-1].x +
	    g_spec->inputOffsetSizes[h_inputRecordCount-1].y;
	  h_inputValsBufSize = g_spec->inputOffsetSizes[h_inputRecordCount-1].z +
	    g_spec->inputOffsetSizes[h_inputRecordCount-1].w;
	  h_inputKeys = g_spec->inputKeys;
	  h_inputVals = g_spec->inputVals;
	  h_inputOffsetSizes = g_spec->inputOffsetSizes;
	...
	if(num == 1) {
	  ...
	  d_inputKeys = g_spec->inputKeys;
	  d_inputVals = g_spec->inputVals;
	  d_inputOffsetSizes = g_spec->inputOffsetSizes;
	}
	...
	CUDA_SAFE_CALL(cudaFree(d_inputKeys));
	cudaFree(d_inputVals);
	cudaFree(d_inputOffsetSizes);	
...
//----------------------------------------------
void startReduce(Spec_t* spec, int num)
{
	Spec_t* g_spec = spec;
...
//param : spec
//----------------------------------------------
void MapReduceSub(Spec_t *spec, int num) {
  assert(NULL != spec);
  Spec_t* g_spec = spec;

  int total_num = g_spec->numMapReduce;
  ...
  if (g_spec->workflow == MAP_GROUP)
    {
      ...
      if (total_num == num && g_spec->outputToHost == 1)
	{
  ...
  if (total_num == num && g_spec->outputToHost == 1)
    {
      ...
      TimeVal_t downloadTv;
      startTimer(&downloadTv);
      ...
      endTimer("PCI I/O (out)", &downloadTv);
...
void MapReduce(Spec_t *spec)
{
  ...
  for(int i = 1; i <= total_num; i++) {
    // do actual i(th) MapReduce
    MapReduceSub(g_spec, i);

    // finalize and initialize for next MapReduce
    if(i < total_num) {
      free(g_spec->inputKeys);
      free(g_spec->inputVals);
      free(g_spec->inputOffsetSizes);
      g_spec->inputRecordCount = 0;

      g_spec->inputKeys = g_spec->outputKeys;
      g_spec->inputVals = g_spec->outputVals;
      g_spec->inputOffsetSizes = g_spec->outputOffsetSizes;
      g_spec->inputRecordCount = g_spec->outputRecordCount;

      g_spec->interRecordCount = 0;
      g_spec->interDiffKeyCount = 0;
      g_spec->interAllKeySize = 0;
      g_spec->interAllValSize = 0;

      g_spec->outputRecordCount = 0;
      g_spec->outputAllKeySize = 0;
      g_spec->outputAllValSize = 0;
      g_spec->outputDiffKeyCount = 0;
