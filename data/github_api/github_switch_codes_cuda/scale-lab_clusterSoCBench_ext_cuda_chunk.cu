// Repository: scale-lab/clusterSoCBench
// File: TeaLeaf/3d/ext_cuda_chunk.cu
...
void ext_init_cuda_(
		...
		int* rank)
{
	Chunks.push_back(new TeaLeafCudaChunk(*xMax, *yMax, *zMax, *rank));
}
	...
	rank(rank)
{
	...
	const int cubeCells = xCells*yCells*zCells;
...
#define CUDA_MALLOC(buf, size) 		\
	...
	CUDA_MALLOC(dU, cubeCells*sizeof(double));
	CUDA_MALLOC(dU0, cubeCells*sizeof(double));
	CUDA_MALLOC(dSd, cubeCells*sizeof(double));
	CUDA_MALLOC(dR, cubeCells*sizeof(double));
	CUDA_MALLOC(dW, cubeCells*sizeof(double));
	CUDA_MALLOC(dP, cubeCells*sizeof(double));
	CUDA_MALLOC(dMi, cubeCells*sizeof(double));
	CUDA_MALLOC(dD, cubeCells*sizeof(double));
	CUDA_MALLOC(dZ, cubeCells*sizeof(double));
	CUDA_MALLOC(dKx, cubeCells*sizeof(double));
	CUDA_MALLOC(dKy, cubeCells*sizeof(double));
	CUDA_MALLOC(dKz, cubeCells*sizeof(double));
	CUDA_MALLOC(dDensity, cubeCells*sizeof(double));
	CUDA_MALLOC(dEnergy0, cubeCells*sizeof(double));
	CUDA_MALLOC(dEnergy1, cubeCells*sizeof(double));
	CUDA_MALLOC(dVolume, cubeCells*sizeof(double));
	CUDA_MALLOC(dCellX, xCells*sizeof(double));
	CUDA_MALLOC(dCellY, yCells*sizeof(double));
	CUDA_MALLOC(dCellZ, zCells*sizeof(double));
	CUDA_MALLOC(dCellDx, xCells*sizeof(double));
	CUDA_MALLOC(dCellDy, yCells*sizeof(double));
	CUDA_MALLOC(dCellDz, zCells*sizeof(double));
	CUDA_MALLOC(dVertexX, (xCells+1)*sizeof(double));
	CUDA_MALLOC(dVertexY, (yCells+1)*sizeof(double));
	CUDA_MALLOC(dVertexZ, (zCells+1)*sizeof(double));
	CUDA_MALLOC(dVertexDx, (xCells+1)*sizeof(double));
	CUDA_MALLOC(dVertexDy, (yCells+1)*sizeof(double));
	CUDA_MALLOC(dVertexDz, (zCells+1)*sizeof(double));
	CUDA_MALLOC(dXArea, (xCells+1)*yCells*zCells*sizeof(double));
	CUDA_MALLOC(dYArea, xCells*(yCells+1)*zCells*sizeof(double));
	CUDA_MALLOC(dZArea, xCells*yCells*(zCells+1)*sizeof(double));
	CUDA_MALLOC(dTopBuffer, (xCells+1)*(zCells+1)*MAX_DEPTH*NUM_FIELDS*sizeof(double));
	CUDA_MALLOC(dBottomBuffer, (xCells+1)*(zCells+1)*MAX_DEPTH*NUM_FIELDS*sizeof(double));
	CUDA_MALLOC(dLeftBuffer, (yCells+1)*(zCells+1)*MAX_DEPTH*NUM_FIELDS*sizeof(double));
	CUDA_MALLOC(dRightBuffer, (yCells+1)*(zCells+1)*MAX_DEPTH*NUM_FIELDS*sizeof(double));
	CUDA_MALLOC(dFrontBuffer, (xCells+1)*(yCells+1)*MAX_DEPTH*NUM_FIELDS*sizeof(double));
	CUDA_MALLOC(dBackBuffer, (xCells+1)*(yCells+1)*MAX_DEPTH*NUM_FIELDS*sizeof(double));

	const size_t reduceBufferLength = 
		std::ceil(1.5*(xCells+1)*(yCells+1)*(zCells+1)/(float)BLOCK_SIZE)*sizeof(double);
	...
	reducePtr1 = thrust::device_ptr<double>(dReduceBuffer1);
	reducePtr2 = thrust::device_ptr<double>(dReduceBuffer2);
	reducePtr3 = thrust::device_ptr<double>(dReduceBuffer3);
	reducePtr4 = thrust::device_ptr<double>(dReduceBuffer4);
