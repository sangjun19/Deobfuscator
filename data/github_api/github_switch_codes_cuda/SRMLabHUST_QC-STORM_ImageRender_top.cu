...
void ImageRenderData_TypeDef::Init(LocalizationPara & LocPara, int MaxDispImgWidth, int MaxDispImgHigh)
{
	...
	*/
	...
	cudaMallocHost((void **)&h_SRImageHist, SRImageHist_DataLen * sizeof(float));
	cudaMalloc((void **)&d_SRImageHist, SRImageHist_DataLen * sizeof(float));
...
//	cudaFreeHost(h_SaveRendImg);
	...
	cudaFreeHost(h_SRImageHist);
	cudaFree(d_SRImageHist);
