// Repository: sitonmoon/Fluid-SPH-CUDA
// File: Source/CUDA/Kernel.cu
...
texture<uint, 1, cudaReadModeElementType> particleHashTex;
...
struct integrate_functor
{
		...
		void operator()(Tuple t)
	{
		...
		thrust::get<2>(t) = make_float4(pos, posData.w);
		thrust::get<3>(t) = make_float4(vel, velData.w);
	}
};
//----------------------------------------------------------------------------------------------------------------------------
///  Boundary Conditions
...
__global__ void computeForceD(float4* newVel, float4* oldPos, float4* oldVel, 
		float4* clr, float* pressure, float* density, float* dyeColor/**/, uint* particleHash, uint* particleIndex, uint* cellStart, uint* cellEnd)
{
		...
		for(int y=-s; y<=s; y++)
		{
			for(int x=-s; x<=s; x++) 
			{
				addVel += compForceCell(gridPos + make_int3(x, y, z), index, pos, vel, oldPos, oldVel,
					pres, dens, pressure, density,	particleHash, cellStart, cellEnd);

				//--- test simple collide
				//addVel += collideCell(gridPos + make_int3(x,y,z) , index, pos, vel, oldPos, oldVel,
				//particleHash, cellStart,cellEnd); //Simple Collider, for test. note:need comment follow code 'v = a*dt' 
				//--- end  
			}
		}
	}
	...
	//  add new vel
	volatile uint si = particleIndex[index];
