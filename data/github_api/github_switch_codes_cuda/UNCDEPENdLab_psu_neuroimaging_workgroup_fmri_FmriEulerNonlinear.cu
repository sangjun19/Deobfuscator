// Repository: UNCDEPENdLab/psu_neuroimaging_workgroup
// File: tapas/mpdcm/src/src/cuda/fmri_FmriEulerNonlinear.cu
...
__device__
FmriEulerNonlinear::FmriEulerNonlinear() : FmriCuda(DIM_FMRI_X, PRELOC_SIZE_FMRI_EULER_X)
{
...
__device__
int FmriEulerNonlinear::update_x(dbuff *ox, dbuff *y, dbuff *u, dbuff *nx)
{
    ...
    if ( maxx < 0 )
    {
        ...
        {  
            case INDEX_X:
                dxacc[toffset] += fmri_fD(ox, y, u, theta, ptheta, j,
                    0, (xnc + 4)/5);
                break;
            case INDEX_F:
                dxacc[toffset + 1] += fmri_fD(ox, y, u, theta, ptheta, j,
                    (xnc + 4)/5, 2 * (xnc + 4)/5);
                break;
            case INDEX_S:
                dxacc[toffset + 2] += fmri_fD(ox, y, u, theta, ptheta, j,
                    2 * (xnc + 4)/5, 3 * ((xnc + 4)/5));
                break;  
            case INDEX_V:
                dxacc[toffset + 3] += fmri_fD(ox, y, u, theta, ptheta, j,
                    3 * ((xnc + 4)/5), 4 * ((xnc + 4)/5));
                break;
            case INDEX_Q:
                dxacc[toffset + 4] += fmri_fD(ox, y, u, theta, ptheta, j,
                    4 * ((xnc + 4)/5), xnc);
                break;   
        }
    }
