...
FROM CommonSettings IMPORT
    (* proc *)  SetINIFilename, SetInitialLanguage, SetFonts, CurrentLanguage;
...
PROCEDURE ["SysCall"] MainDialogueProc(hwnd     : OS2.HWND
    ...
    CONST UpdateSemName = "\SEM32\WEBSERVE\UPDATED";
...
PROCEDURE CreateMainDialogue (LocalRemote: CARDINAL; TNImode: BOOLEAN);
    ...
    BEGIN
        ...
        RemoteFlag := Remote.InitialSetup (lang, "WebServe", "Setup",
                                              "D:\Servers\WebServe", UseTNI);
        SetLabels (lang);
