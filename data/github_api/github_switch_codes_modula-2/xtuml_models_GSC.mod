...
// File: masl/test/GSC/GSC.mod

//
// UK Crown Copyright (c) 2019. All rights reserved.
//

//! The Gas Station Control domain manages the dispensing of
//! fuel, customer payments and tank levels.
//! 
...
//! closing a micro switch, the clutch is engaged and fuel
//! pumped.  When it is released, the clutch is freed.  There is
//! also a micro switch on the holster in which the gun is kept
//! which prevents fuel being pumped until the gun is taken out.
//!  Once the gun is replaced in the holster, the delivery is
//! deemed to be completed and the pump disabled.  Further
//! depressions of the trigger in the gun cannot dispense more
//! fuel.  After a short standby period, the pump motor will be
//! turned off unless the pump is re-enabled.
//! 
//! A metering device in the fuel line sends a pulse to the
...
//! user to dispense fuel ad lib.  The sophisticated pumps,
//! imported from New Zealand, allow the customer to preset an
//! amount and a volume of fuel.  fuel will then be pumped up to
//! a maximum of the required quantity.
//! 
...
//! 
//! At present, two grades of fuel are dispensed from five pumps
//! on the forecourt.  Each pump takes its supply from one of
//! two tanks, one tank for each grade.  The tank level must not
//! drop below 4% of the tank's capacity.  If this happens, the
//! pumps serviced by that tank cannot be enabled to dispense
//! fuel.
//! 
domain GSC is
  object DELIVERY;
  ...
  private type TankType is enum (Standard,
                                 Large);

  private type currency_type is real;

  private type my_structure_type is structure
    cost     : real;
    duration : integer;
    aWEDASDF : currency_type;
  end structure;

  //! Represents garage attendant interface
  terminator ATTENDANT is
    //! This solicits permission to pump gas from a particular pump.
    //!  The permission is obtained from the Attendant through some
    //! user interface
  ...
  private service Prototype_Petrol_Station_1 (); pragma scenario (1);
  ...
  object DELIVERY is
    ...
    Volume_Delivered : preferred real;

    Cost             : preferred currency_type;


    identifier is (Time, Pump_Number, Volume_Delivered);


    public service Create_Delivery (Pump_Id : in  integer);

    state Pending ();

    state Calculating_price_of_delivery ();

    terminal state Delivery_complete ();
    ...
    transition is
      Non_Existent (                 Fuel_Unit_Delivered => Cannot_Happen,
                                     Delivery_Complete   => Cannot_Happen,
                                     Delete_Delivery     => Cannot_Happen);
      Pending (                      Fuel_Unit_Delivered => Calculating_price_of_delivery,
                                     Delivery_Complete   => Delivery_Cancelled,
                                     Delete_Delivery     => Ignore);
      Calculating_price_of_delivery (Fuel_Unit_Delivered => Calculating_price_of_delivery,
                                     Delivery_Complete   => Delivery_complete,
                                     Delete_Delivery     => Ignore);
      Delivery_complete (            Fuel_Unit_Delivered => Cannot_Happen,
                                     Delivery_Complete   => Cannot_Happen,
                                     Delete_Delivery     => Cannot_Happen);
      Delivery_Cancelled (           Fuel_Unit_Delivered => Cannot_Happen,
                                     Delivery_Complete   => Cannot_Happen,
                                     Delete_Delivery     => Cannot_Happen);
    end transition;
  ...
  object TRANSACTION is
    ...
    Transaction_Number       : preferred integer;
    ...
    Transaction_Process_Time : duration;

    //! The time the delivery commenced, this shall be copied from
    //! the DELIVERY Object.
    Delivery_Start_Time      : duration;


    public service Create_Transaction (Delivery_Time : in  duration,
                                       Delivery_Pump : in  integer,
                                       Delivery_Cost : in  currency_type);

    state Created ();

    state Paid ();

    state Evaded (Customer_Details : in  string);
    ...
    transition is
      Non_Existent (Payment_Received  => Cannot_Happen,
                    Customer_Absconds => Cannot_Happen,
                    Transaction_paid  => Cannot_Happen);
      Created (     Payment_Received  => Paid,
                    Customer_Absconds => Evaded,
                    Transaction_paid  => Cannot_Happen);
      Paid (        Payment_Received  => Cannot_Happen,
                    Customer_Absconds => Cannot_Happen,
                    Transaction_paid  => Cannot_Happen);
      Evaded (      Payment_Received  => Paid,
  ...
  pragma persistence ("Bean Managed");

  object TYPE_OF_TANK is
