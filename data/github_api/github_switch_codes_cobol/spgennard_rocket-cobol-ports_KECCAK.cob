...
*> linkage for the STATE_PERMUTE module
 01 LNK-STATE_PERMUTE.
...
*>  Absorb all the input blocks
    PERFORM UNTIL WS-INPUT-BYTE-LEN <= 0
       ...
       THEN
          MOVE WS-STATE TO LNK-STATE OF LNK-STATE_PERMUTE
          CALL "STATE_PERMUTE" USING LNK-STATE_PERMUTE END-CALL
          MOVE LNK-STATE OF LNK-STATE_PERMUTE TO WS-STATE
...
*> linkage for the LFSR86540 module
 ...
 PROCEDURE DIVISION USING LNK-STATE_PERMUTE.

*>------------------------------------------------------------------------------
 MAIN-STATE_PERMUTE SECTION.
*>------------------------------------------------------------------------------
...
*>     Compute the theta effect for a given column
       ...
       CALL "ROL_LANE" USING LNK-ROL-LANE END-CALL
...
*>     Add the theta effect to the whole column   
       PERFORM VARYING WS-Y FROM 0 BY 1 UNTIL WS-Y > 4
          ...
          MOVE LNK-STATE OF LNK-STATE_PERMUTE 
            TO LNK-STATE OF LNK-XOR-LANE
          MOVE WS-D 
            TO LNK-LANE  OF LNK-XOR-LANE
          CALL "XOR_LANE" USING LNK-XOR-LANE END-CALL
          MOVE LNK-STATE OF LNK-XOR-LANE
            TO LNK-STATE OF LNK-STATE_PERMUTE
       END-PERFORM
    END-PERFORM
...
*>------------------------------------------------------------------------------
    ...
    CALL "READ_LANE" USING LNK-READ-LANE END-CALL
...
*>     Swap current and state(x,y), and rotate
       ...
       CALL "WRITE_LANE" USING LNK-WRITE-LANE END-CALL
