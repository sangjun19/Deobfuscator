// Repository: YDHCUI/rustdesk
// File: agent_src/src/server/connection.rs
...
impl Connection {
    pub async fn start(
        ...
        tokio::spawn(async move {
            if let Err(_) = start_ipc(rx_to_cm, tx_from_cm).await {
        ...
        if let Some(message::Union::login_request(lr)) = msg.union {
            ...
            if lr.username != Config::get_id() {
                ...
                let mut failure = LOGIN_FAILURES
                    ...
                    .map(|x| x.clone())
                    .unwrap_or((0, 0, 0));
                let time = (crate::common::get_time() / 60_000) as i32;
                if failure.2 > 30 {
            ...
            if t.from_client {
                ...
                let latency = crate::common::get_time() - t.time;
                if latency > 0 {
                ...
                Some(message::Union::key_event(mut me)) => {
                    if self.keyboard {
                        ...
                        let is_press = if cfg!(target_os = "linux") {
                            (me.press || me.down) && !crate::common::is_modifier(&me)
                        } else {
                            me.press
                        };
                        if is_press {
    ...
    fn on_close(&mut self, _reason: &str, lock: bool) {
        if let Some(s) = self.server.upgrade() {
    ...
    if let Ok(s) = crate::ipc::connect(1000, "_cm").await {
        ...
        if !run_done {
            super::CHILD_PROCESS
                .lock()
                .unwrap()
                .push(crate::common::run_me(vec!["--cm"])?);
        }
        for _ in 0..10 {
            sleep(0.3).await;
            if let Ok(s) = crate::ipc::connect(1000, "_cm").await {
