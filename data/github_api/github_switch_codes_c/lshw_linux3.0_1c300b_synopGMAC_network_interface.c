...
s32 synopGMAC_check_phy_init (synopGMACPciNetworkAdapter * gmacdev);
...
static const struct net_device_ops loongson_gmac_netdev_ops = {
...
synopGMACdevice            *gmacdev = adapter->synopGMACdev;
	...
	if(!gmac_get_link(adapter->synopGMACnetdev))
	{
		...
		printk(" gmac_get_link  return speed100M...............\r\n");
		return 0;
	}
	else
...
void synop_handle_received_data(struct net_device *netdev)
{
	...
	TR("<0>desc_index=%d\n",desc_index);
		...
		if(desc_index >= 0 && data1 != 0){
			...
			dma_unmap_single(dev,dma_addr1,len,PCI_DMA_FROMDEVICE);
...
{       
	...
	s32 status;
	u32 dma_addr;
//	printk("synopGMAC_intr_handler==%d\n",intr_num);
//	dump_stack();
...
//	TR("phy reg16 is 0x%0x \n",data);
	...
	printk("haha data=0x%x\n", data);
#if 0
	if(data == 0x181)
	{
		printk(" phy:DM9161A ............dbg-yg.........\r\n");
		
	    data = 0x100;    // set RMII mode
        synopGMAC_write_phy_reg(gmacdev->MacBase,gmacdev->PhyBase,0x10,data);
        synopGMAC_read_phy_reg(gmacdev->MacBase,gmacdev->PhyBase,0x10,&data);
...
//	disable_irq_nosync(adapter->irq);//fsy
...
#else
	synopGMAC_dma_bus_mode_init(gmacdev, DmaBurstLength4 | DmaDescriptorSkip2);                      //pbl32 incr with rxthreshold 128
//	synopGMAC_dma_bus_mode_init(gmacdev, DmaResetOn | DmaDescriptorSkip2);                      //pbl32 incr with rxthreshold 128
#endif
...
void * __init synopGMAC_init_network_interface(struct device *dev,u8* synopGMACMappedAddr,u32 synopGMACMappedAddrSize,int irq)
{
	...
	*/
	...
	netdev->netdev_ops	= &loongson_gmac_netdev_ops;  
