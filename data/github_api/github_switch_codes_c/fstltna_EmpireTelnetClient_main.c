...
print_usage(char *program_name)
{
    printf("Usage: %s [OPTION]...[COUNTRY [PASSWORD]]\n"
	   ...
	   "  -t              Display connection text file begfore login\n"
	   "  -T              Activate Telnet mode\n"
	   "  -u              Use UTF-8\n"
...
main(int argc, char **argv)
{
    ...
    FILE *fptr;
    char c;
    char filename[] = "/usr/local/var/empire/connmess";;

    while ((opt = getopt(argc, argv, "2:H:krs:uhvtT")) != EOF) {
	switch (opt) {
	case '2':
	    auxfname = optarg;
	    break;
