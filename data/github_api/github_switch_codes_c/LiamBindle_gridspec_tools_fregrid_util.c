			...
			unsigned int opcode)
{
  ...
  for(l=0; l<nscalar; l++) for(n=0; n<ntiles; n++) {
    ...
    scalar[n].var[l].interp_method = interp_method;
  }
  ...
  for(l=0; l<nvector; l++) for(n=0; n<ntiles; n++) {
    ...
    u_comp[n].var[l].interp_method = interp_method;
    v_comp[n].var[l].nz            = 1;
    ...
    v_comp[n].var[l].interp_method = interp_method;
  }
  ...
  for(m=0; m<nfiles; m++) {
	...
	if(cart[0] == 'Z' || cart[1] == 'Z' || cart[0] == 'N' || cart[1] == 'N') {
	  field[n].var[ll].has_zaxis = 1;
	  field[n].var[ll].nz        = (cart[0] == 'Z' || cart[0] == 'N') ? dimsize[0]:dimsize[1];
			  ...
			  const char *history, const char *tagname)
{
  ...
  for(n=0; n<ntiles_out; n++) {
    for(l=0; l<nscalar; l++) {
      ...
      scalar_out[n].var[l].interp_method = scalar_in[0].var[l].interp_method;
    ...
    for(l=0; l<nvector; l++) {
      ...
      u_out[n].var[l].interp_method = u_in[0].var[l].interp_method;
      ...
      v_out[n].var[l].interp_method = v_in[0].var[l].interp_method;
    }
  }
