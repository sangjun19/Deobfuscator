// Repository: mestradam/hyplas
// File: src/ALGORITHM/algwsd.f
...
!
        if (nalgo.lt.0) then
! Set up prescribed displacements for tangential solution for the
! arc-length method
          call tangen
        end if
!
...
!
! For Arc-Length method only: Compute iterative displacement according
! to the arc-length constraint and update the incremental and total
! load factors
! --------------------------------------------------------------------
          if(nalgo.lt.0)then
            call arclen
...
!
            if(inccut)then
! no real roots for arc-length constraint equation: break equilibrium
! iteration loop and activate increment cutting
              goto 30
            endif
          endif
!
...
!        
!       Perform calculations to the discontinuity for the next step
!       -----------------------------------------------------------
        call discon
!
...
!
        if(convrg)then
!       Iterations have converged: break equilibrium iteration loop and
