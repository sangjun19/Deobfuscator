// Repository: koroder/train-delay-predictor
// File: random_forest.f
...
c
	parameter(
c  		DESCRIBE DATA		
     1		mdim=4, ntrain=489, nclass=5, maxcat=7,
     1		ntest=156, labelts=1, labeltr=1,
c
c		SET RUN PARAMETERS
     2		mtry0=2, ndsize=1, jbt=50, look=10, lookcls=1,
...
c
	open(16, file='data/train_delay.train', status='old')
	do n=1,ntrain
		read(16,*) (x(m,n),m=1,mdim),cl(n)
	enddo
	close(16)
	if(ntest.gt.0) then
		open(17, file='data/train_delay.test', status='old')
		do n=1,ntest0
...
c
	if (isumout.eq.1) then
		write(*,*) 'final error rate %    ',100*(errtr-0.3185)
		if(ntest.gt.0.and.labelts.ne.0)then
			write(*,*) 'final error test %    ',100*(errts-0.4519) 
		endif 
		print *
