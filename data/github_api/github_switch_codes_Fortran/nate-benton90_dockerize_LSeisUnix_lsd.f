// Repository: nate-benton90/dockerize_LSeisUnix
// File: sioseis-2024.1.1/lsd.f

!    A PROGRAM TO READ AND LIST A SEGY FORMATTED DISK
!  -K = list Knudsen header items.
!  -O = Odec - NOT IMPLEMENTED
!
!  mod 5 Feb 14 - honor segy rev 1 delay scalar
!  mod ??????   - honor segy rev 1 water bottom scalar
!  mod 28 Oct 14 - Replace iargc with command_argument_count() 
!  mod 28 Oct 14 - Replace getarg with command_argument() 
!  mod 24 Mar 14 - Finish some never finished changes on coordinates
!  mod Jan 2022 - change order and length of CHARACTER*100 name, token
!                 because diskio.c mkname might have a memory overwrite
!                 problem that appears on Macs and gcc 9. 
! mod 20 Apr 2022 - Change getfil(5,  to getfil( 4,    What in H is 5?
!
      INTEGER*2 ibuf
      REAL*8 dtemp
!     might have 65K samples/words
      DIMENSION BUF(70000),IBUF(222),LBUF(222)
      CHARACTER*150 name, token
      ...
      numarg = 1
      CALL get_command_argument( numarg, name, length, istat)
      IF( name(1:1) .EQ. '-' ) THEN
     ...
     &      ' lsd [-K] SEGY-filename [trace-inc ntodo long-list]'
      ...
      IF( nargs .GT. numarg ) THEN
          CALL get_command_argument( numarg, token, length, istat)
          numarg = numarg + 1
          READ( token, * ) nskip
          IF( nargs .GT. numarg ) THEN
              CALL get_command_argument( numarg, token, length, istat)
...
!****            = 4, then +/-DDDMMSS.ss
     ...
     &                    '(',temp,'arcsecs)'
              ELSEIF( ibuf(45) .EQ. 3 .OR. ibuf(45) .EQ. 5 ) THEN
!****   3 means INTEGER decimal degrees, but I haven't seen that, but ODEC has floats
!****   5 is what I want to see for FLOAT decimal degrees
     ...
     &                    ibuf(36)
                  ...
                  IF( ibuf(36) .GT. 0 )
     &                dtemp19 = dtemp19 * DFLOAT(ibuf(36))
                  IF(ibuf(36) .LT. 0)
     &                dtemp19=DABS(dtemp19/DFLOAT(ibuf(36)))
                  dtemp20 = DABS(DFLOAT(lbuf(20)))
                  IF( ibuf(36) .GT. 0 )
     &                dtemp20 = dtemp20 * DFLOAT(ibuf(36))
                  IF(ibuf(36) .LT. 0)
     &                dtemp20=DABS(dtemp20/DFLOAT(ibuf(36)))
                  PRINT *,' Source coordinates: ', dtemp19, dtemp20
                  dtemp21 = DABS(DFLOAT(lbuf(21)))
                  IF( ibuf(36) .GT. 0 )
     &                dtemp21 = dtemp21 * DFLOAT(ibuf(36))
                  IF(ibuf(36) .LT. 0)
     &                dtemp21=DABS(dtemp21/DFLOAT(ibuf(36)))
                  dtemp22 = DABS(DFLOAT(lbuf(22)))
                  IF( ibuf(36) .GT. 0 )
     &                dtemp22 = dtemp22 * DFLOAT(ibuf(36))
                  IF(ibuf(36) .LT. 0)
     &                dtemp22=DABS(dtemp22/DFLOAT(ibuf(36)))
     ...
     &               ibuf(36)
          ...
          IF( knudsen ) THEN
!****         Valid on raw files only.  sioseis diskox doesn't swap the same way.
     ...
     &            temp = - FLOAT(lbuf(58)) / FLOAT(ibuf(114))
      ...
      ENDIF
      IF( nsamps .GT. 0 ) THEN
!         read rather than skip so short traces show up
