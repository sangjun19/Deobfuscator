// Repository: hep-mirrors/POWHEG-BOX
// File: VBF_W-Z/convert_coup.f
...
c strong coupling from powheg:
      ...
      print *,"-----------------------------------------------"
      xmh=vbfnloinput('#HMASS')
      if(xmh.lt.0d0) xmh=126d0
      xgh=-999d0
      XGH=vbfnloinput('#HWIDTH')
      if (replace(xmt,0)) xmt=vbfnloinput('#TOPMASS')
      if(xmt.lt.0d0) xmt=172.4d0
      if (replace(xmb,0)) xmb= vbfnloinput("#BOTTOMMASS")
      if(xmb.lt.0d0) xmb=4.855d0
      if (replace(xmc,0)) xmc=vbfnloinput("#CHARMMASS")
      if(xmc.lt.0d0) xmc=1.65d0
      if (replace(xmtau,0)) xmtau=vbfnloinput("#TAU_MASS")
      if(xmtau.lt.0d0) xmtau=1.77684d0

      ewscheme=int(vbfnloinput("#EWSCHEME"))
      ...
      Case(1)
         ...
         if(gf.lt.0d0) gf=1.16637d-5
         if (replace(alfa,0)) then 
            alfa= vbfnloinput("#INVALFA")
            if(alfa.lt.0d0) alfa=128.944341122D0
            alfa=1/alfa
         endif
         if (replace(xmz,0)) xmz= vbfnloinput("#ZMASS")
         if(xmz.lt.0d0) xmz=91.1876d0
         xmw = -1.d0
         sin2w = -1.d0
      CASE(2)
         if (replace(gf,0)) gf= vbfnloinput("#FERMI_CONST")
         ...
         if(sin2w.lt.0d0) sin2w=0.222646d0
         if (replace(xmz,0)) xmz= vbfnloinput("#ZMASS")
         if(xmz.lt.0d0) xmz=91.1876d0
         xmw = -1.d0
         alfa = -1.d0
      ...
      alfas= pwhg_alphas(xmz**2,st_lambda5MSB,st_nlight)!alphas5(BMASS_POLE**2,1)alphas5(xmz**2D0,1)
! 
      anom1= int(vbfnloinput("ANOM_CPL") )
      anom=.false.
      if (anom1.eq.1) anom=.true.


      kkswitch=.false.


      if(anom .and. kkswitch) then
      Print*, "********************************"//
     &        "*********************************" 
      Print*, "Clash: Both ANOM_CPL and KK_MOD "//
...
** Setting the electroweak parameters
     ...
     -           HjjjLO, HjjjLO_AA, HjjjLO_mu, HjjjLO_tau, HjjjLO_bbar, 
     -           HjjjLO_WW, HjjjLO_ZZ_ll, HjjjLO_ZZ_lnu)
            with_anomHiggs=anom
            call read_anomHVV
            anom_called = .true.
         END SELECT
...
!       print*, xmmu, mm
      if (replace(bhmu,3) .or. replace(xgh,3)) then
         XMmu = 0.105658367d0         !  and  H ----> mu+ mu-
!          xmmu = MM
...
c$$$      IF (INFO.LE.1) RETURN
c$$$      WRITE(6,18) XGW,BWNE*100.d0,BWUD*100.d0,BWTB*100.d0,
c$$$     &            XGZ,BZNN*100.d0,BZEE*100.d0,BZUU*100.d0,
c$$$     &                BZDD*100.d0,BZTT*100.d0,
c$$$     &            XGH,BHWW*100.d0,BHZZ*100.d0,BHGG*100.d0,BHTT*100.d0,
c$$$     &                BHBB*100.d0,BHCC*100.d0,BHMU*100.d0,BHTAU*100.d0,
c$$$     &            BHGAM*100.d0,BHGAMZ*100.0d0
c$$$   18 FORMAT("THE WIDTHS OF THE HEAVY BOSONS, AND THE WAY",
c$$$     & " THEY ARE BUILT UP:"/,
c$$$     & " TOTAL WIDTH OF W+ OR W-       =",F7.3," GEV"/,
c$$$     & " BR FOR LEPTON DECAY           =",F7.3," %"/,
c$$$     & " BR FOR MASSLESS QUARK DECAY   =",F7.3," %"/,
c$$$     & " BR FOR TOP-BOTTOM DECAY       =",F7.3," %"/,
c$$$     & " TOTAL WIDTH OF Z0             =",F7.3," GEV"/,
c$$$     & " BR FOR NEUTRINO DECAY         =",F7.3," %"/,
c$$$     & " BR FOR LEPTON DECAY           =",F7.3," %"/,
c$$$     & " BR FOR LIGHT UP DECAY         =",F7.3," %"/,
c$$$     & " BR FOR DOWN DECAY             =",F7.3," %"/,
c$$$     & " BR FOR TOP DECAY              =",F7.3," %"/,
c$$$     & " TOTAL WIDTH OF HIGGS          =",F9.5," GEV"/,
c$$$     & " BR FOR WW DECAY               =",F7.3," %"/,
c$$$     & " BR FOR ZZ DECAY               =",F7.3," %"/,
c$$$     & " BR FOR GLUE DECAY             =",F7.3," %"/,
c$$$     & " BR FOR TOP DECAY              =",F7.3," %"/,
c$$$     & " BR FOR B DECAY                =",F7.3," %"/,
c$$$     & " BR FOR CHARM DECAY            =",F7.3," %"/,
c$$$     & " BR FOR MU DECAY               =",F7.3," %"/,
c$$$     $ " BR FOR TAU DECAY              =",F7.3," %"/,
c$$$     & " BR FOR PHOTON DECAY           =",F7.3," %"/
c$$$     & " BR FOR Photon Z decay         =",F7.3," %")
c$$$
c$$$      write(6,"(a,f6.2,a)")" TOTAL WIDTH OF TOP QUARK      =",
c$$$     &     gamt," GeV"
c$$$
c$$$      if (.false.) then
c$$$      write(6,19) (XM2(J),XMG(J),J=1,6)
c$$$      endif
c$$$   19 FORMAT(/," MASS**2 AND MASS*WIDTH VALUES"/,6(2F10.2/))
c$$$      write(6,20)
c$$$   20 FORMAT(20(1H-)," END OF KOPPEL INFO   ",20(1H-))
c$$$   21 FORMAT(//" ",63("*"),/2(" *",61(" "),"*"/)," *       ",A,5(" "),
c$$$     &       "*"/,2(" *",61(" "),"*"/)," ",63("*") )
c$$$
