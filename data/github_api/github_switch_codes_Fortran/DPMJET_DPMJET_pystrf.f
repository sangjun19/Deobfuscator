...
C...Search for very nearby partons that may be recombined.
  ...
  140 IF(NR.GT.NRMIN) THEN
        PDRMIN=2D0*PARU12
        DO 150 I=N+1,N+NR
          IF(I.EQ.N+NR.AND.ABS(K(N+1,2)).NE.21) GOTO 150
          I1=I+1
          IF(I.EQ.N+NR) I1=N+1
          IF(K(I,1).EQ.41.OR.K(I1,1).EQ.41) GOTO 150
          IF(MJU(1).NE.0.AND.I1.LT.MJU(1).AND.ABS(K(I1,2)).NE.21)
     &    GOTO 150
          IF(MJU(2).NE.0.AND.I.GT.MJU(2).AND.ABS(K(I,2)).NE.21)
     &    GOTO 150
...
C...Junction strings: generate flavour, hadron, pT, z and Gamma.
  440     CALL PYKFDI(KFL(1),0,KFL(3),K(I,2))
          ...
          IF(IRANKJ.EQ.1.AND.ABS(KFL(1)).LE.10.AND.
     &    ABS(KFL(3)).GT.10) THEN
          ...
          IF(ABS(KFL(1)).GE.4.AND.ABS(KFL(1)).LE.8.AND.
     &    MSTU(90).LT.8) THEN
            MSTU(90)=MSTU(90)+1
            MSTU(90+MSTU(90))=I
            PARU(90+MSTU(90))=Z
...
C...Junction strings: put together to new effective string endpoint.
        ...
        KFJS(JT)=SIGN(1000*MAX(ABS(KFJH(1)),ABS(KFJH(2)))+
     &  100*MIN(ABS(KFJH(1)),ABS(KFJH(2)))+KFLS,KFJH(1))
        DO 630 J=1,4
...
C...Find longitudinal string directions (i.e. lightlike four-vectors).
      DO 700 IS=1,NS
        ...
        DO 680 J=1,5
          DP(1,J)=P(IS1,J)
          IF(ABS(K(IS1,2)).EQ.21) DP(1,J)=0.5D0*DP(1,J)
          IF(IS1.EQ.MJU(1)) DP(1,J)=PJS(1,J)-PJS(3,J)
          DP(2,J)=P(IS2,J)
          IF(ABS(K(IS2,2)).EQ.21) DP(2,J)=0.5D0*DP(2,J)
...
C...Initialize flavour and pT variables for open string.
      IF(NS.LT.NR) THEN
        ...
        DO 760 JT=1,2
          ...
          IF(MJU(JT).NE.0.AND.ABS(KFL(JT)).GT.1000) IBARRK(JT)=1
          MSTJ(93)=1
...
C.. MOPS variables
      DO 800 JT=1,2
         XTMO(JT)=1D0
         PM2QMO(JT)=PMQ(JT)**2
         IF(ABS(KFL(JT)).GT.10) PM2QMO(JT)=0D0
  800 CONTINUE
...
C.. New side priority for popcorn systems
      IF(MSTU(121).LE.0)THEN
         JT=1.5D0+PYR(0)
         IF(ABS(KFL(3-JT)).GT.10) JT=3-JT
         IF(ABS(KFL(3-JT)).GE.4.AND.ABS(KFL(3-JT)).LE.8) JT=3-JT
      ENDIF
      JR=3-JT
      JS=3-2*JT
      IRANK(JT)=IRANK(JT)+1
...
C...Generate flavour, hadron and pT.
  880 K(I,3)=IE(JT)
      ...
      IF(IRANK(JT).EQ.1.AND.ABS(KFL(JT)).LE.10.AND.
     &ABS(KFL(3)).GT.10) THEN
        IF(PYR(0).GT.PARJ(19)) GOTO 880
      ENDIF
      IF(IBARRK(JT).EQ.1.AND.MOD(ABS(K(I,2)),10000).GT.1000)
     &K(I,3)=IJUORI(JT)
      P(I,5)=PYMASS(K(I,2))
...
C...Final hadrons for small invariant mass.
      ...
      IF(ABS(KFL(JT)).GT.10.AND.ABS(KFL(3)).GT.10) WMIN=
...
C...Choose z, which gives Gamma. Shift z for heavy flavours.
      ...
      IF(ABS(KFL(JT)).GE.4.AND.ABS(KFL(JT)).LE.8.AND.
     &MSTU(90).LT.8) THEN
        MSTU(90)=MSTU(90)+1
        MSTU(90+MSTU(90))=I
        PARU(90+MSTU(90))=Z
...
C.. MOPS baryon model modification
      XTMO3=(1D0-Z)*XTMO(JT)
      IF(ABS(KFL(3)).LE.10) NRVMO=0
      IF(ABS(KFL(3)).GT.10.AND.MSTJ(12).GE.4) THEN
         GTSTMO=1D0
         PTSTMO=1D0
         RTSTMO=PYR(0)
         IF(ABS(KFL(JT)).LE.10)THEN
...
C.. MOPS Accepting popcorn system hadron.
         IF(PTSTMO*GTSTMO.GT.RTSTMO) THEN
            IF(IRANK(JT).EQ.1.OR.ABS(KFL(JT)).LE.10) THEN
               NRVMO=I-N-NR
...
C..Lift restoring string outside MOPS block
  910 IF(MSTU(121).LT.0) THEN
         ...
         MSTU(90)=MU90MO
         NRVMO=0
         IF(IRANK(JT).EQ.1.OR.ABS(KFL(JT)).LE.10) GOTO 880
         I=IMO
         KFL(JT)=KFLMO
         PMQ(JT)=PMQMO
         PX(JT)=PXMO
...
C...Final hadron: side, flavour, hadron, mass.
 1080 I=I+1
      ...
      IF(K(I,2).EQ.0) GOTO 710
      IF(IBARRK(JT).EQ.1.AND.MOD(ABS(K(I-1,2)),10000).GT.1000)
     &IBARRK(JT)=0
      IF(IBARRK(JT).EQ.1.AND.MOD(ABS(K(I,2)),10000).GT.1000)
     &K(I,3)=IJUORI(JT)
      IF(IBARRK(JR).EQ.1.AND.MOD(ABS(K(I,2)),10000).GT.1000)
     &K(I,3)=IJUORI(JR)
      P(I,5)=PYMASS(K(I,2))
