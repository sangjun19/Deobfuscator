// Repository: tinghuagithub/Anomalous-Higgs-Boson-Couplings
// File: VBFNLO/loops/Integrals/FF/ff/ffxc0i.f

*--#[ log:
*	$Id: vbfffxc0i.f,v 1.3 1996/06/03 12:11:43 gj Exp $
*	$Log: vbfffxc0i.f,v $
c Revision 1.3  1996/06/03  12:11:43  gj
c Added an error message for vbfffxc0j with zero masses, which is ill-defined.
c
...
*  #[ check input:
	if ( init .eq. 0 .and. .not.lsmug ) then
	    init = 1
	    print *,'vbfffxc0i: infra-red divergent threepoint function, ',
     +		'working with a cutoff ',delta
	endif
	if ( .not.lsmug .and. delta .eq. 0 ) then
	    call vbffferr(59,ier)
	    return
	endif
	if ( lwrite ) then
*	    print input
	    print *,'vbfffxc0i: infrared divergent threepoint function'
	    if ( .not.lsmug ) then
...
*
	call vbfffrot3(irota,xqi,dqiqj,qiDqj,xpi,dpipj,dum66,6,3,3,ier)
*
*	get some dotproducts
*
	if ( ldot ) then
	    call vbfffdot3(qiDqj,xqi,dqiqj,6,ier)
	    do 5 i=1,6
		do 4 j=1,6
	...
	if ( lwarn .and. abs(qiDqj(4,1)) .lt. xloss*xmax )
     +		call vbfffwarn(156,ier,qiDqj(4,1),xmax)
	...
	if ( lwarn .and. abs(qiDqj(4,2)) .lt. xloss*xmax )
     +		call vbfffwarn(156,ier,qiDqj(4,2),xmax)
	...
	if ( lwarn .and. abs(qiDqj(1,2)) .lt. xloss*xmax )
     +		call vbfffwarn(156,ier,qiDqj(1,2),xmax)
...
*
	if ( del2 .gt. 0 ) then
	    call vbffferr(83,ier)
	    return
	endif
*
	sdel2 = isgnal*sqrt(-del2)
*
	call vbfffxc0j(cs,ipi12,sdel2,clogi,ilogi,xqi,dqiqj,qiDqj,
     +		delta,3,ier)
*  #] calculations:
*  #[ sum:
*
...
*
   ...
   10	continue
	...
	if ( lwarn .and. 2*absc(csum) .lt. xloss*xmax ) then
	    call vbfffwarn(157,ier,absc(csum),xmax)
	endif
...
*###[ vbfffxc0j:
	subroutine vbfffxc0j(cs,ipi12,sdel2i,clogi,ilogi,
...
*
	...
	DOUBLE COMPLEX clog1,clog2,cdum(2),cel3,cdyzm,cdyzp,cli,chulp,
     +		carg1,carg2,chulp1
	DOUBLE COMPLEX vbfzfflog,vbfzxfflg,cc
	DOUBLE PRECISION del2,zm,zp,zm1,zp1,sdel2,hulp,xheck,dum(3),
     +		vbfdfflo1,dyzp,dyzm,wm,wp,absc,arg1,arg2,del3
*
*	common blocks etc
*
	include 'ff.h'
...
*  #[ check input:
	if ( ltest ) then
	    call vbfffxhck(xpi,dpipj,6,ier)
	endif
	if ( lwrite ) then
	    print '(a)','  ##[ vbfffxc0j:'
	    print *,'vbfffxc0j: input: '
	    print *,'xpi   = ',xpi
	    if ( .not.lsmug ) then
		print *,'delta = ',delta
	    else
...
*
	...
	if ( sdel2i .gt. 0 .and. lwrite ) print *,
     +		'vbfffxc0j: cannot handle sdel2>0, switched to sdel2<0'
*
	if ( xpi(4).eq.0 ) then
	    zm = xpi(2)/dpipj(2,1)
	    zm1 = -xpi(1)/dpipj(2,1)
	else
	    call vbfffroot(zm,zp,xpi(4),piDpj(4,2),xpi(2),sdel2,ier)
	    if ( dpipj(1,2) .ne. 0 ) then
		call vbfffroot(zp1,zm1,xpi(4),-piDpj(4,1),xpi(1),sdel2,ier)
	    else
		zm1 = zp
		zp1 = zm
	    endif
	...
	if ( ltest ) then
     ...
     +		'vbfffxc0j: zm + zm1 <> 1: ',zm,zm1,xheck
	    if ( xpi(4).ne.0 ) then
		xheck = zp + zp1 - 1
		if ( xloss*abs(xheck) .gt. precx*max(x1,abs(zp)) )
     +		    print *,'vbfffxc0j: zp + zp1 <> 1: ',zp,zp1,xheck
	    endif
	endif
...
*
	if ( xpi(4).ne.0 ) then
	    call vbfffcxr(cs(1),ipi12,zm,zm1,zp,zp1,dyzp,
...
*
		...
		clog1 = vbfdfflo1(1/zm1,ier)
	    elseif ( zm.ne.0 ) then
		clog1 = vbfzxfflg(-zm/zm1,-2,x0,ier)
	    else
	        call vbffferr(97,ier)
	        return
	    endif
...
*
	    ier0 = ier
	    if ( hulp.eq.0 ) call vbffferr(97,ier)
	    clog2 = vbfzxfflg(hulp,2,x0,ier0)
	    cs(8) = -clog1*clog2/2
	    if ( lwrite ) then
...
*
	    ...
	    carg1 = +cdyzm*DBLE(1/zm)
	     arg1 = +dyzm/zm
	    clog1 = vbfzfflog(-carg1,+ieps,DCMPLX(DBLE(zm),DBLE(0)),ier)
...
*		ier = ier + 50
	    ...
	    carg2 = -cdyzm*DBLE(1/zm1)
	     arg2 = -dyzm/zm1
	    clog2 = vbfzfflog(-carg2,ieps,DCMPLX(DBLE(-zm1),DBLE(0)),ier)
...
*  #[ log(1) for npoin=4:
	if ( npoin .eq. 4 ) then
		...
		if ( .not.lsmug ) then
		    ...
		    ier0 = ier
		    if ( hulp.eq.0 ) call vbffferr(97,ier)
		    clogi(1) = -vbfzxfflg(abs(hulp),0,x0,ier0)
		    if ( hulp .lt. 0 ) then
			if ( xpi(4) .gt. 0 ) then
			    ilogi(1) = -1
			else
			...
			if ( ltest ) then
			    print *,'vbfffxc0j: I am not 100% sure of the',
     ...
     +				' limit lam->0 (id=',id,')'
		    ...
		    if ( xpi(4).eq.0 ) then
			print *,'vbfffxc0i: cannot handle t=0 yet, sorry'
     ...
     +			call vbfffwarn(129,ier,absc(chulp1),x1)
		    call vbfffxclg(clogi(1),ilogi(1),chulp,chulp1,dyzp,
     +			ier)
		endif
	    endif
	endif
...
*
     ...
     +	    		.gt.precx*delta**2 ) then
		if ( xpi(1) .ne. delta ) then
		    ier0 = ier
		    if ( xpi(1).eq.0 ) call vbffferr(97,ier)
		    cs(9) = -vbfzxfflg(xpi(1)/delta,0,x0,ier0)**2 /4
		endif
		if ( xpi(2) .ne. delta ) then
		    ier0 = ier
		    if ( xpi(2).eq.0 ) call vbffferr(97,ier)
		    cs(10) = vbfzxfflg(xpi(2)/delta,0,x0,ier0)**2 /4
		endif
	    endif
	    if ( lwrite ) then
		print *,'cs(9)= ',cs(9)
...
*  #[		real case:
		if ( lwrite ) print *,'vbfffxc0i: Real S3'
*
*		first z-,z+
*
...
*		the (di)logs
*
		clog1 = vbfzxfflg(-dyzp,-ieps,x1,ier)
		cs(10) = -clog1**2/2
		ipi12 = ipi12 - 4
		clog2 = vbfzxfflg(-dyzm,+ieps,x1,ier)
		cs(11) = -clog2**2/2
		ipi12 = ipi12 - 2
		hulp = dyzp/dyzm
		if ( dyzp .lt. 0 ) then
		...
		call vbfffzxdl(cli,i,cdum(1),hulp,+ieps1,ier)
		cs(12) = -cli
		ipi12 = ipi12 - i
*
*		the log for npoin=4
*
		if ( npoin.eq.4 ) then
			...
			call vbfffxclg(clogi(3),ilogi(3),chulp,chulp1,dyzp,
     +				ier)
		    endif
		endif
*
...
*  #[		complex case:
		if ( lwrite ) print *,'vbfffxc0i: Complex S3'
*
*		first z+
*
		cdyzp = -cmipj(1,3)*DBLE(wm)/(2*DBLE(xpi(6))) -
     +			cmipj(2,2)/(2*DBLE(sdel2))
		clog1 = vbfzfflog(-cdyzp,-ieps,c1,ier)
...
*
     ...
     +			cmipj(2,2)/(2*DBLE(sdel2))
		clog2 = vbfzfflog(-cdyzm,+ieps,c1,ier)
...
*
		...
		if ( DIMAG(chulp) .eq. 0 ) then
		    ...
		    call vbfffzzdl(cli,i,cdum(1),chulp,ier)
			...
			if ( lwrite ) then
			    ...
			    print *,'cli was ',cli
			    print *,'cli is  ',cli +
     +				ieps1*c2ipi*vbfzfflog(chulp,0,c0,ier)
			    call vbfffzxdl(cdum(2),i,cdum(1),hulp,+ieps1,
     +				ier)
			    print *,'vgl   ',cdum(2)
			endif
			cli = cli + ieps1*c2ipi*vbfzfflog(chulp,0,c0,ier)
		    endif
		endif
		cs(12) = -cli
		ipi12 = ipi12 - i
...
*
     ...
     +			DBLE(cmipj(1,3))/(2*DBLE(sdel2))
		clog2 = vbfzxfflg(+dyzp,+ieps,x1,ier)
		cs(14) = +clog2**2/2
		ipi12 = ipi12 + 2
		hulp = dyzm/dyzp
		if ( dyzm .lt. 0 ) then
...
*
		if ( npoin.eq.4 ) then
			...
			call vbfffxclg(clogi(2),ilogi(2),chulp,chulp1,dyzp,
     +				ier)
		    endif
		endif
*
...
*  #[		complex case:
		if ( lwrite ) print *,'vbfffxc0i: Complex S2'
*
*		first z-
*
		cdyzm = -cmipj(2,2)*DBLE(wp)/(2*DBLE(xpi(5))) -
     +			cmipj(1,3)/(2*DBLE(sdel2))
		clog1 = vbfzfflog(+cdyzm,-ieps,c1,ier)
...
*
     ...
     +			cmipj(1,3)/(2*DBLE(sdel2))
		clog2 = vbfzfflog(+cdyzp,+ieps,c1,ier)
...
*  #[ sdel2<0!:
   ...
   20	    	continue
	...
	if ( lwrite ) print '(a)','  ##] vbfffxc0j:'
*  #] sdel2<0!:
*###] vbfffxc0j:
	end
*###[ vbfffxclg:
	subroutine vbfffxclg(clg,ilg,chulp,chulp1,dyzp,ier)
***#[*comment:***********************************************************
*									*
...
*	local variables
*
	DOUBLE PRECISION hulp,hulp1,vbfdfflo1
	DOUBLE COMPLEX vbfzxfflg,vbfzfflog,vbfzfflo1,check
*
*	common blocks
*
	include 'ff.h'
...
*  #[ check:
	if ( ltest ) then
     ...
     +		print *,'vbfffxclg: error: chulp1 != 1+chulp: ',chulp1,
     +		c1+chulp,check
	endif
*  #] check:
*  #[ work:
*
	if ( DIMAG(chulp) .eq. 0 ) then
		...
		clg = DBLE(vbfdfflo1(hulp1,ier))
	    else
		clg = vbfzxfflg(abs(hulp),0,x0,ier)
	    endif
	    if ( hulp .lt. 0 ) then
		if ( dyzp.lt.0 ) then
		    ilg = +1
...
*
		...
		clg = vbfzfflo1(chulp1,ier)
	    else
		clg = vbfzfflog(chulp,0,c0,ier)
	    endif
	    ilg = 0
	    if ( DBLE(chulp) .lt. 0 ) then
		if ( dyzp.lt.0 .and. DIMAG(clg).lt.0 ) then
		    if ( lwrite ) print *,'vbfffxclg: added -2*pi to log'
		    ilg = +2
