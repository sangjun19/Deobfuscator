// Repository: cms-externals/MCFM
// File: src/ZZ/qqb_zz_v.f
...
C----calculate ocer limited flavor range -2:2
     ...
     &       *(prop12*v2(pol1)*r(j)+q2*q(j))*qqb2(polq,pol1,pol2))
      ...
      virt=virt-2d0*facnlo*ave*dble(
     &           dconjg(aqqb)*bqqb_SAVE(j,k,polq,pol1,pol2)
     &          +dconjg(bqqb)*aqqb_SAVE(j,k,polq,pol1,pol2))
     ...
     &       *(prop12*v2(pol1)*r(k)+q2*q(k))*qbq2(polq,pol1,pol2)
      ...
      elseif (ii.eq.2) then
         if (pol1.eq.pol2) then
      virt=virt-2d0*facnlo*ave
     &       *dble(dconjg(aqbq)*bqbq_SAVE(j,k,polq,pol1,pol2)
     &            +dconjg(bqbq)*aqbq_SAVE(j,k,polq,pol1,pol2))
         endif
      endif
