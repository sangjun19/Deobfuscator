...
if ( use_simulated .or. use_ideal_obs ) then
   call rd_truth_wrf(filename,ix,jx,kx,ni,nj,nk,nv,nm,gid,sid,iid,jid,xt)
   ...
   do i=i1,i1+2
     if(i>=istart.and.i<=iend.and.j>=jstart.and.j<=jend) then
        if(use_simulated) then
          tmpsend(i-i1+1,j-j1+1,:,:)=xt(i-istart+1,j-jstart+1,:,:)
        else
...
obs_cycle: do ig=1,ceiling(real(obs%num)/nob)
...
if ( use_simulated .or. use_ideal_obs ) then
  ...
  if (my_proc_id==0) then
    do iob=1,obs%num
      ...
      y(iob) = obs%dat(iob) + obs%err(iob)*gaussdev(sum(values))
    end do
  end if
  call MPI_Bcast(y,obs%num,MPI_REAL,0,comm,ierr)
else
  y=obs%dat
end if
...
! skip those ya outside update zone
      ...
      do ie=1,numbers_en+1
         ...
         ya(iiob,ie)=ya(iiob,ie)+corr_coef*fac*cov*(y(iob)-ya(iob,numbers_en+1))/d        !mean
