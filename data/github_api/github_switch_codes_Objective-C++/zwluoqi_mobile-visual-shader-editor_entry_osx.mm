// Repository: zwluoqi/mobile-visual-shader-editor
// File: src/common/entry/entry_osx.mm
...
namespace entry
{
	...
	struct Context
	{
        ...
        bool processRequestEvent(const Event* event){
            if (event)
            {
                switch (event->m_type)
                {
                    case Event::DialogOpen:
                    {
                        const DialogOpenEvent* dialog = static_cast<const DialogOpenEvent*>(event);
                        nfdchar_t *outPath;
                        nfdresult_t openDialogResult;
                        if(dialog->m_filePath.getCPtr()[0]=='n'){
                    ...
                    break;
                    case Event::DialogSave:
                    {
                        const DialogOpenEvent* dialog = static_cast<const DialogOpenEvent*>(event);

                        nfdchar_t *outPath;
                       nfdfilteritem_t filterItem[1] = { { "NodeShader", "nodedata" }};
                        nfdresult_t openDialogResult = NFD_SaveDialogN(&outPath, filterItem, 1, NULL, NULL);
                        if ( openDialogResult == NFD_OKAY )
                        {
                            m_eventQueue.postSaveFileEvent(dialog->m_handle, outPath,0);
                            printf("Success!");
                            NFD_FreePath(outPath);
                        }
                        else if ( openDialogResult == NFD_CANCEL )
                        {
                            m_eventQueue.postSaveFileEvent(dialog->m_handle, NULL,1);
                            printf("User pressed cancel.");
                        }
                        else
                        {
                            m_eventQueue.postSaveFileEvent(dialog->m_handle, NULL,2);
                            printf("Error: %s\n", NFD_GetError() );
                        }
                    }
                        break;
...
//            [path stringByAppendingString:@"/"];
			...
			while (!(m_exit = [dg applicationHasTerminated]) )
			{
				...
				@autoreleasepool
				{
                    ...
                    const Event* m_ev =  m_requestQueue.poll() ;
                    while (processRequestEvent(m_ev))
                    {
                        m_requestQueue.release(m_ev);
                        m_ev =  m_requestQueue.poll() ;
                    }
				}
			}
    ...
    bool _postDialogOpenEvent(const bx::FilePath &_filePath){
        WindowHandle handle = { 0 };
        s_ctx.m_requestQueue.requestDialogOpenEvent(handle, _filePath);
        return true;
    }

    bool _postDialogSaveEvent(const bx::FilePath &_filePath){
        WindowHandle handle = { 0 };
        s_ctx.m_requestQueue.requestDialogSaveEvent(handle, _filePath);
        return true;
    }
