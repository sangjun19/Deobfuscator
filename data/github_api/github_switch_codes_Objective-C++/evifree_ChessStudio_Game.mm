// Repository: evifree/ChessStudio
// File: ChessStudio/ChessStudio/Engines/Stockfish/Controller/Game.mm
...
- (id)initWithGameController:(GameController *)gc PGNString:(NSString *)string {
   ...
   char name[PGN_STRING_SIZE], value[PGN_STRING_SIZE];
    ...
    //NSLog(@"INITWITHGAMECONTROLLER: %@", string);
...
static NSString* breakLinesInString(NSString *string) {
   ...
   for (i = 0; i < [array count]; i++) {
      NSUInteger length = [[array objectAtIndex: i] length];
      if (j + length + 1 < 80) {
         if (i > 0) { // HACK
            [mstr appendString: @" "];
            j += length + 1;
...
- (void)computeOpeningString {
   ...
   for (NSInteger i = [moves count]; i >= 0; i--) {
       ...
       //NSLog(@"COMPUTE OPENING STRING I=%d", i);
       
      NSString *s = [[ECO sharedInstance] openingDescriptionForKey:
                                             currentPosition->get_key((int)i)];
       //int k = currentPosition->get_key((int)i);
       //NSLog(@"KEY OPENING STRING %d", k);
       //NSLog(@"OPENING STRING = %@", s);
      if (s != nil) {
         [openingString release];
         openingString = [s retain];
         break;
      }
   }
}
