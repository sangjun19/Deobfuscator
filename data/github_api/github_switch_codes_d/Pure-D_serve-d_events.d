// Repository: Pure-D/serve-d
// File: serverbase/source/served/utils/events.d
...
{
	deprecated("This is no longer needed after upgrading `extension.modules` to `memberModules`")
	// string[] allowedDuplicateMethods = ["object"];
...
mixin template EventProcessor(alias ExtensionModule, EventProcessorConfig config = EventProcessorConfig.init)
{
	...
	else static if (__traits(hasMember, ExtensionModule, "members"))
		mixin LegacyEventProcessorCoreImpl!(ExtensionModule, config);
	else
...
deprecated("This is only used with `alias members`, upgrade to `alias memberModules`")
mixin template LegacyEventProcessorCoreImpl(alias ExtensionModule, EventProcessorConfig config = EventProcessorConfig.init)
{
